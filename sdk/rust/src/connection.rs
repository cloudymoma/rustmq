...
impl Connection {
    ...
    pub async fn new(config: &ClientConfig) -> Result<Self> {
        info!(
            "Creating new QUIC connection with {} brokers",
            config.brokers.len()
        );
        ...
        let (security_manager, security_context) = if let Some(auth_config) = &config.auth {
                ...
                .unwrap_or(false)
            {
                let security_manager =
                    Arc::new(SecurityManager::new(auth_config.security.clone().unwrap())?);
                ...
                let context = security_manager
                    .authenticate(tls_config, auth_config)
                    .await?;
    ...
    async fn create_endpoint(config: &ClientConfig) -> Result<Endpoint> {
        debug!(
            "Creating QUIC endpoint with TLS enabled: {}",
            config.enable_tls
        );
        ...
        transport_config
            ...
            .max_idle_timeout(Some(config.keep_alive_interval.try_into().map_err(
                |_| ClientError::InvalidConfig("Invalid keep alive interval".to_string()),
            )?))
        ...
        let mut endpoint =
            Endpoint::client("0.0.0.0:0".parse().map_err(|e| {
    ...
    fn create_tls_client_config(config: &ClientConfig) -> Result<QuinnClientConfig> {
        ...
        let rustls_config = if let Some(auth_config) = &config.auth {
            if auth_config
                ...
                .unwrap_or(false)
            {
                let security_manager = SecurityManager::new(auth_config.security.clone().unwrap())?;
        ...
        let crypto = quinn::crypto::rustls::QuicClientConfig::try_from(rustls_config)
            .map_err(|e| ClientError::Tls(format!("Failed to create QUIC client config: {}", e)))?;
    ...
    fn load_client_private_key(tls_config: &TlsConfig) -> Result<PrivateKeyDer<'static>> {
        ...
        let mut reader = std::io::BufReader::new(client_key_pem.as_bytes());
        let keys: std::result::Result<Vec<_>, _> =
            rustls_pemfile::rsa_private_keys(&mut reader).collect();
        if let Ok(mut keys) = keys {
            if !keys.is_empty() {
                return Ok(PrivateKeyDer::from(keys.remove(0)));
            }
    ...
    fn create_insecure_client_config() -> Result<QuinnClientConfig> {
        ...
        let crypto = quinn::crypto::rustls::QuicClientConfig::try_from(crypto).map_err(|e| {
            ClientError::Tls(format!(
                "Failed to create insecure QUIC client config: {}",
                e
            ))
        })?;
    ...
    async fn establish_connections(
        ...
        for broker_addr_str in &config.brokers {
            ...
            connection_futures.push(async move {
                Self::connect_with_retry(&endpoint, addr, &server_name, connect_timeout, config)
                    .await
                    .map(|conn| (addr, conn))
            });
        }
    ...
    pub async fn get_connection(&self) -> Result<QuicConnection> {
        ...
        if let Some(error) = entry.connection.close_reason() {
            ...
            tokio::spawn(async move {
                if let Err(e) =
                    Self::reconnect_broker(connections_clone, endpoint_clone, config_clone, addr)
                        .await
                {
            ...
            return Err(ClientError::BrokerNotAvailable {
    ...
    async fn send_request_internal(&self, request: Vec<u8>) -> Result<Vec<u8>> {
        ...
        let send_result = timeout(self.config.request_timeout, send_stream.write_all(&frame)).await;
        ...
        match send_result {
            ...
            Ok(Err(e)) => {
                return Err(ClientError::QuicTransport(format!(
        ...
        if type_byte[0] == 255 {
            ...
            let error_data = recv_stream
                ...
                .map_err(|e| match e {
                    ...
                    quinn::ReadToEndError::Read(read_err) => ClientError::QuicTransport(format!(
                        "Failed to read error data: {}",
                        read_err
                    )),
                })?;
    ...
    pub async fn send_message(&self, message: Vec<u8>) -> Result<()> {
        ...
        match send_result {
            ...
            Ok(Err(e)) => Err(ClientError::QuicTransport(format!(
    ...
    pub async fn send_messages_batch(&self, messages: Vec<Vec<u8>>) -> Result<Vec<Result<()>>> {
        ...
        for chunk in messages.chunks(batch_size) {
            ...
            for message in chunk {
                ...
                futures.push(async move { connection_clone.send_message(message).await });
            }
    ...
    pub async fn is_connected(&self) -> bool {
        ...
        !connections.is_empty()
            && connections
                .values()
                .any(|entry| entry.connection.close_reason().is_none())
    }
    ...
    pub async fn health_check(&self) -> Result<bool> {
        ...
        for (addr, entry) in connections.iter() {
            ...
            check_futures
                .push(async move { Self::check_connection_health(connection, addr).await });
        }
        ...
        info!(
            "Health check completed: {}/{} brokers healthy ({}%)",
            healthy_count,
            connections.len(),
            (health_ratio * 100.0) as u32
        );
    ...
    async fn cleanup_failed_connections(&self) {
        ...
        for (addr, entry) in connections.iter() {
            if entry.connection.close_reason().is_some() {
                ...
                if error_count > 5 {
                    // Remove after 5 consecutive errors
                    to_remove.push(*addr);
                }
            }
        }
    ...
    pub async fn stats(&self) -> ConnectionStats {
        ...
        let broker_stats: Vec<BrokerConnectionInfo> = connections
            .iter()
...
impl Connection {
    ...
    pub async fn refresh_security_context(&self) -> Result<()> {
        if let (Some(security_manager), Some(auth_config)) =
            (&self.security_manager, &self.config.auth)
        {
            let default_tls_config = TlsConfig::default();
            let tls_config = self
                .config
                .tls_config
                .as_ref()
                .unwrap_or(&default_tls_config);
            let new_context = security_manager
                .authenticate(tls_config, auth_config)
                .await?;
