...
impl MockBrokerServer {
    async fn new() -> Result<Self> {
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .map_err(|e| ClientError::Connection(format!("Failed to bind test server: {}", e)))?;
...
async fn test_connection_retry_logic() {
    ...
    assert!(
        result.is_err(),
        "Expected connection to fail with non-existent broker"
    );

    // Should take some time due to retries (at least one retry)
    assert!(
        elapsed >= config.retry_config.base_delay,
        "Should take at least base_delay for retry"
    );
}
