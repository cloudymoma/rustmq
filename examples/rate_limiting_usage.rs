...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Per-IP RPS limit: {}", rate_config.per_ip.requests_per_second);
...
mod tests {
    ...
    fn test_endpoint_categorization() {
        ...
        assert_eq!(categorize_endpoint("/api/v1/cluster", &config), Some("read_operations"));
        assert_eq!(categorize_endpoint("POST:/api/v1/topics", &config), Some("write_operations"));
        assert_eq!(categorize_endpoint("/api/v1/cluster/rebalance", &config), Some("cluster_operations"));
    }
