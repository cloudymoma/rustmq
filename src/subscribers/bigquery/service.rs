...
use crate::subscribers::bigquery::{
    client::{BigQueryWriter, create_bigquery_writer},
    config::{BigQuerySubscriberConfig, DeadLetterAction, RetryBackoffStrategy},
    error::{BigQueryError, Result},
    types::{
        BigQueryBatch, BigQueryMessage, ConnectionStatus, HealthState, HealthStatus, InsertResult,
        MessageMetadata, SubscriberMetrics,
    },
};
...
pub trait RustMqConsumer: Send + Sync {
    ...
    async fn start_consuming(
        &mut self,
    ) -> Result<Box<dyn Stream<Item = Result<RawMessage>> + Unpin + Send>>;
    ...
    fn validate_config(config: &BigQuerySubscriberConfig) -> Result<()> {
        ...
        if config.subscription.topic.is_empty() {
            return Err(BigQueryError::Config(
                "subscription topic is required".to_string(),
            ));
        }
        ...
        if config.batching.max_rows_per_batch == 0 {
            return Err(BigQueryError::Config(
                "max_rows_per_batch must be greater than 0".to_string(),
            ));
        }

        if config.batching.max_batch_size_bytes == 0 {
            return Err(BigQueryError::Config(
                "max_batch_size_bytes must be greater than 0".to_string(),
            ));
        }
    ...
    fn start_metrics_updater(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if total_messages > 0 {
                    health.error_rate =
                        (metrics_read.messages_failed as f64 / total_messages as f64) * 100.0;
                }
...
impl MessageProcessor {
        ...
        if let Ok(permit) = self.batch_semaphore.try_acquire() {
            ...
            if !messages_to_process.is_empty() {
                ...
                for message in messages_to_process {
                    batch.add_message(message);
                    if batch.is_full(
                        batch_config.max_rows_per_batch,
                        batch_config.max_batch_size_bytes,
                    ) {
                        break;
                    }
                }
    ...
    fn calculate_retry_delay(&self, attempt: usize) -> Duration {
        match &self.config.error_handling.retry_backoff {
            ...
            RetryBackoffStrategy::Fixed { delay_ms } => Duration::from_millis(*delay_ms),
        }
    }
    ...
    async fn handle_dead_letter(&self, batch: &BigQueryBatch) {
        match &self.config.error_handling.dead_letter_action {
            DeadLetterAction::Drop => {
                warn!(
                    "Dropping {} messages from failed batch {}",
                    batch.messages.len(),
                    batch.metadata.batch_id
                );
            }
