...
use super::*;
use crate::config::*;
use crate::types::{Header, Record};
use bytes::Bytes;
use filter::{ConditionalRuleEngine, TopicFilterEngine};
...
async fn test_comprehensive_topic_filtering() {
    ...
    let exact_filters = vec![TopicFilter {
    ...
    let wildcard_filters = vec![TopicFilter {
    ...
    // Test regex filter
    let regex_filters = vec![TopicFilter {
    ...
    // Test prefix filter
    let prefix_filters = vec![TopicFilter {
    ...
    // Test suffix filter
    let suffix_filters = vec![TopicFilter {
    ...
    let contains_filters = vec![TopicFilter {
...
async fn test_conditional_rule_evaluation() {
    ...
    let header_rules = vec![ConditionalRule {
    ...
    assert!(
        engine
            .evaluate_record(&record_with_header, "test.topic")
            .unwrap()
    );

    let record_without_header =
        Record::new(Some(b"test".to_vec()), b"{}".to_vec(), vec![], 1234567890);
    assert!(
        !engine
            .evaluate_record(&record_without_header, "test.topic")
            .unwrap()
    );

    // Test payload field condition
    let payload_rules = vec![ConditionalRule {
    ...
    assert!(
        engine
            .evaluate_record(&record_with_payload, "test.topic")
            .unwrap()
    );

    // Test message size condition
    let size_rules = vec![ConditionalRule {
...
async fn test_wasm_instance_pool() {
    ...
    // Test pre-warming
    pool.prewarm_module("prewarm-module", &module_config)
        .await
        .unwrap();
    let stats = pool.get_global_stats();
    assert!(
...
async fn test_backward_compatibility() {
    ...
    let result = processor
        .process_record(record, vec![module_id.clone()])
        .await
        .unwrap();
...
async fn test_processor_orchestrator_integration() {
    ...
    let record = create_test_record();
    let result = processor
        .process_record_with_topic(record, "test.topic")
        .await
        .unwrap();
...
async fn test_runtime_pipeline_management() {
    ...
    processor_with_orchestrator
        .add_pipeline(new_pipeline_config)
        .await
        .unwrap();

    // Test removing pipeline at runtime
    processor_with_orchestrator
        .remove_pipeline("runtime-pipeline")
        .await
        .unwrap();
}
...
async fn test_performance_characteristics() {
    ...
    for _ in 0..10 {
        let results = orchestrator
            .execute_pipelines(record.clone(), "test.topic")
            .await
            .unwrap();
        assert_eq!(results.len(), 1);
        assert!(results[0].success);
