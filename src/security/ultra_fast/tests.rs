...
async fn create_test_system() -> UltraFastAuthSystem {
    ...
    UltraFastAuthSystem::new(config, acl_config, metrics)
        .await
        .unwrap()
}
...
async fn test_performance_targets_validation() {
    ...
    let system = UltraFastAuthSystem::new(config, acl_config, metrics)
        .await
        .unwrap();
...
async fn test_simd_vs_scalar_consistency() {
    ...
    let individual_results: Vec<_> = keys.iter().map(|key| system.authorize_fast(key)).collect();
...
async fn test_performance_under_load() {
    ...
    for run in 0..3 {
        ...
        println!(
            "Performance test run {}: {:.0} ops/sec",
            run + 1,
            ops_per_sec
        );
    ...
    assert!(
        best_ops_per_sec > 800_000.0,
        "Performance too low: {:.0} ops/sec (best of 3 runs)",
        best_ops_per_sec
    );
