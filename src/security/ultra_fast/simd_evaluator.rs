...
use crate::security::ultra_fast::{
    UltraFastAuthResult,
    compact_encoding::{BatchPermissionEncoder, encode_permission},
};
...
impl BranchlessEvaluator {
    ...
    pub fn check_permissions_batch(actual_perms: &[u32], required_perms: &[u32]) -> Vec<bool> {
        actual_perms
            .iter()
...
mod tests {
    ...
    fn test_branchless_evaluation() {
        ...
        assert!(BranchlessEvaluator::check_permission_branchless(
            actual,
            required_read
        ));
        assert!(!BranchlessEvaluator::check_permission_branchless(
...
mod simd_tests {
    ...
    fn test_avx2_permission_checking() {
        ...
        unsafe {
            let results =
                BatchPermissionEncoder::check_permissions_simd(&actual_perms, &required_perms);

            // Check results match scalar computation
            let scalar_results =
                BatchPermissionEncoder::check_permissions_scalar(&actual_perms, &required_perms);
