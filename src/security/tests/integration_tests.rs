...
mod tests {
    ...
    use crate::security::acl::{AclManager, AclOperation, AclRule, ResourceType};
    use crate::security::auth::authorization::AclCacheEntry;
    use crate::security::auth::{AclKey, AuthorizationManager, Permission};
    use crate::security::tls::*;
    use crate::security::*;
    use crate::types::TopicName;
    // Use certificate manager types for certificate creation
    use crate::security::tls::certificate_manager::{
        CaGenerationParams, CertificateRequest, CertificateRole as CertMgrRole, KeyType,
    };
    ...
    async fn create_integrated_security_system() -> (SecurityManager, TempDir) {
        ...
        config.certificate_management.ca_cert_path =
            temp_dir.path().join("ca.pem").to_string_lossy().to_string();
        config.certificate_management.ca_key_path =
            temp_dir.path().join("ca.key").to_string_lossy().to_string();

        // The key fix: Create SecurityManager with a custom constructor that accepts storage path
        let security_manager = SecurityManager::new_with_storage_path(config, temp_dir.path())
            .await
            .unwrap();
    ...
    async fn test_end_to_end_authentication_flow() {
        ...
        subject.push(
            ...
            "integration-test-client".to_string(),
        );
        subject.push(
            rcgen::DnType::OrganizationName,
            "RustMQ Test Client".to_string(),
        );

        let cert_request = CertificateRequest {
            subject,
            role: CertMgrRole::Client,
            san_entries: vec![rcgen::SanType::DnsName("client.test.local".to_string())],
        ...
        let client_cert = security_manager
            .certificate_manager()
        ...
        let principal = security_manager
            .authentication()
        ...
        assert_eq!(
            auth_context.certificate_fingerprint,
            Some(client_cert.fingerprint)
        );
        ...
        let validation_result = security_manager
            .authentication()
            .validate_certificate(&der_data)
            .await;
        assert!(
            validation_result.is_ok(),
            "Certificate validation should succeed, error: {:?}",
    ...
    async fn test_complete_authorization_pipeline() {
        ...
        for rule in acl_rules {
            ...
            let cache_entry =
                AclCacheEntry::new(rule.effect == Effect::Allow, Duration::from_secs(300));
        ...
        let allowed_key = AclKey::new(principal.clone(), "allowed-topic".into(), Permission::Read);

        let denied_key = AclKey::new(principal.clone(), "denied-topic".into(), Permission::Read);

        let allowed_result = security_manager
            .authorization()
            .check_authorization(&allowed_key)
            .await;
        let denied_result = security_manager
            .authorization()
            .check_authorization(&denied_key)
            .await;

        assert!(allowed_result.is_ok(), "Authorization check should succeed");
        assert!(
            allowed_result.unwrap(),
            "Should be allowed for allowed-topic"
        );
    ...
    async fn test_certificate_lifecycle_integration() {
        ...
        subject.push(
            rcgen::DnType::CommonName,
            "broker-001.cluster.local".to_string(),
        );
        ...
        let broker_cert = security_manager
            .certificate_manager()
            .issue_certificate(cert_request)
            .await
            .unwrap();
        ...
        let certificate = rustls_pki_types::CertificateDer::from(der_data);
        let fingerprint = "test_fingerprint";
        let auth_principal = security_manager
            .authentication()
            .extract_principal_from_certificate(&certificate, fingerprint)
            .unwrap();
        ...
        let renewed_cert = security_manager
            .certificate_manager()
            .renew_certificate(&broker_cert.id)
            .await
            .unwrap();

        assert_ne!(
            broker_cert.id, renewed_cert.id,
            "Renewed certificate should have different ID"
        );
        assert_eq!(
            broker_cert.role, renewed_cert.role,
            "Renewed certificate should have same role"
        );

        // Refresh revoked certificates list in authentication manager
        security_manager
            .authentication()
            .refresh_revoked_certificates()
            .await
            .unwrap();

        // 5. Verify original certificate is revoked
        let original_status = security_manager
            .certificate_manager()
            .get_certificate_status(&broker_cert.id)
            .await
            .unwrap();
        assert_eq!(
            original_status,
        ...
        let is_revoked = security_manager
            .authentication()
            .is_certificate_revoked(&broker_cert.fingerprint)
            .await
            .unwrap();
        assert!(
            is_revoked,
            "Original certificate should be detected as revoked"
        );

        let renewed_is_revoked = security_manager
            .authentication()
            .is_certificate_revoked(&renewed_cert.fingerprint)
            .await
            .unwrap();
        assert!(
            !renewed_is_revoked,
            "Renewed certificate should not be revoked"
        );
    }
    ...
    async fn test_security_metrics_integration() {
        ...
        let der_data = rustls_pemfile::certs(&mut pem_data.as_bytes())
            ...
            .next()
            .unwrap();
        let _auth_result = security_manager
            .authentication()
            .validate_certificate(&der_data)
            .await;
        ...
        let _failed_result = security_manager
            .authentication()
        ...
        let _authz_result = security_manager
            .authorization()
            .check_authorization(&auth_key)
            .await;
        ...
        assert!(
            final_metrics.auth_success_count > initial_auth_success,
            "Authentication success count should increase"
        );
        assert!(
            final_metrics.auth_failure_count > initial_auth_failure,
            "Authentication failure count should increase"
        );
        assert!(
            final_metrics.authz_success_count > initial_authz_checks,
            "Authorization checks count should increase"
        );

        // 5. Test metrics aggregation
        let total_auth_attempts =
            final_metrics.auth_success_count + final_metrics.auth_failure_count;
        assert!(
            total_auth_attempts > initial_auth_success + initial_auth_failure,
    ...
    async fn test_security_audit_logging_integration() {
        ...
        let auth_key = AclKey::new(principal.clone(), "audit-topic".into(), Permission::Read);
        ...
        assert!(
            operations.contains(&"generate_root_ca"),
            "Should log CA generation"
        );
        assert!(
            operations.contains(&"issue_certificate"),
            "Should log certificate issuance"
        );
        assert!(
            operations.contains(&"revoke_certificate"),
            "Should log certificate revocation"
        );
        ...
        for i in 0..10 {
            ...
            let _test_cert = security_manager
                .certificate_manager()
                .issue_certificate(cert_request)
                .await
                .unwrap();
        ...
        let extended_audit_entries = security_manager
            .certificate_manager()
            .get_recent_audit_entries(150)
            .await
            .unwrap();

        assert!(
            extended_audit_entries.len() > initial_log_size,
            "Audit log should contain additional entries (initial: {}, final: {})",
            initial_log_size,
            extended_audit_entries.len()
        );
    }
    ...
    async fn test_security_configuration_hot_updates() {
        ...
        let (initial_tls_config, initial_acl_config) = {
            ...
            assert!(
                initial_config.tls.enabled,
                "TLS should be enabled initially"
            );
            assert!(
                initial_config.acl.enabled,
                "ACL should be enabled initially"
            );
        ...
        let tls_update_result = security_manager
            .update_tls_config(new_tls_config.clone())
            .await;
        assert!(
            tls_update_result.is_ok(),
            "TLS config update should succeed"
        );

        let updated_config = security_manager.get_current_config();
        assert!(
            matches!(
                updated_config.tls.mode,
                crate::security::tls::TlsMode::Server
            ),
            "TLS config should be updated"
        );
        ...
        let acl_update_result = security_manager
            .update_acl_config(new_acl_config.clone())
            .await;
        assert!(
            acl_update_result.is_ok(),
            "ACL config update should succeed"
        );

        let final_config = security_manager.get_current_config();
        assert_eq!(
            final_config.acl.cache_ttl_seconds, 600,
            "ACL cache TTL should be updated"
        );
        assert_eq!(
            final_config.acl.bloom_filter_size, 20_000,
            "Bloom filter size should be updated"
        );
    ...
    async fn test_cross_component_error_propagation() {
        ...
        let auth_result = security_manager
            .authentication()
            .validate_certificate(invalid_cert_data)
            .await;

        assert!(
            auth_result.is_err(),
            "Invalid certificate should cause authentication error"
        );
        ...
        match auth_result.unwrap_err() {
            ...
            other => panic!(
                "Expected InvalidCertificate or CertificateValidation, got {:?}",
                other
            ),
        }
        ...
        let authz_result = security_manager
            .authorization()
            .check_authorization(&auth_key)
            .await;
        ...
        assert!(
            authz_result.is_ok(),
            "Missing ACL rules should be handled gracefully"
        );
        assert!(
            !authz_result.unwrap(),
            "Missing ACL rules should deny access"
        );
        ...
        let revocation_check = security_manager
            .authentication()
            .is_certificate_revoked(&client_cert.fingerprint)
            .await;

        assert!(revocation_check.is_ok(), "Revocation check should succeed");
        assert!(
            revocation_check.unwrap(),
            "Revoked certificate should be detected"
        );
    }
    ...
    async fn test_concurrent_security_operations() {
        ...
        let final_metrics = security_manager.metrics().snapshot();
        assert!(
            final_metrics.auth_success_count > 0,
            "Should have successful authentications"
        );
        assert!(
            final_metrics.authz_success_count > 0,
    ...
    async fn test_end_to_end_webpki_authentication_flow() {
        ...
        let auth_result =
            test_certificate_authentication_webpki(&security_manager, &client_cert).await;

        assert!(
            auth_result.is_ok(),
            "End-to-end WebPKI authentication should succeed: {:?}",
            auth_result
        );
        ...
        let webpki_validation =
            test_webpki_specific_features(&security_manager, &client_cert).await;
        assert!(
            webpki_validation.is_ok(),
            "WebPKI-specific validation should succeed: {:?}",
            webpki_validation
        );
    }
    ...
    async fn test_comprehensive_webpki_certificate_lifecycle() {
        ...
        let certificate_scenarios = vec![
            ...
            (
                "client-webpki",
                CertMgrRole::Client,
                vec![rcgen::SanType::DnsName("client.rustmq.local".to_string())],
            ),
            (
                "controller-webpki",
                CertMgrRole::Controller,
                vec![
        ...
        for (name, role, san_entries) in certificate_scenarios {
            ...
            subject.push(
                rcgen::DnType::OrganizationName,
                "WebPKI Integration Test".to_string(),
            );
            ...
            let cert_result = security_manager
                .certificate_manager()
                .issue_certificate(cert_request)
                .await
                .unwrap();
            ...
            assert!(
                validation_result.is_ok(),
                "WebPKI validation should succeed for {}: {:?}",
                name,
                validation_result
            );
            ...
            let principal = security_manager
                .authentication()
                .extract_principal_with_webpki(&certificate, "webpki_test")
                .unwrap();
            assert!(
                principal.contains(name),
                "WebPKI principal should contain certificate name for {}",
                name
            );
    ...
    async fn test_webpki_certificate_chain_validation_integration() {
        ...
        for (i, cert) in certificates.iter().enumerate() {
            ...
            let comprehensive_validation = security_manager
                .authentication()
                .validate_certificate_comprehensive(&der_data)
                .await;
            assert!(
                comprehensive_validation.is_ok(),
                "WebPKI comprehensive validation should succeed for certificate {}: {:?}",
                i,
        ...
        let cert_ders: Vec<_> = certificates
            .iter()
            .map(|cert| {
                let pem_data = cert.certificate_pem.clone().unwrap();
        ...
        let batch_results = security_manager
            .authentication()
            .validate_certificates_batch(&cert_der_refs)
            .await;

        assert_eq!(
            batch_results.len(),
            certificates.len(),
            "Batch validation should process all certificates"
        );
        for (i, result) in batch_results.iter().enumerate() {
            assert!(
                result.is_ok(),
                "Batch validation should succeed for certificate {}: {:?}",
                i,
                result
            );
        }
    ...
    async fn test_webpki_performance_integration() {
        ...
        for cert in &test_certificates {
            ...
            let _validation_result = security_manager
                .authentication()
                .validate_certificate_comprehensive(&der_data)
                .await
                .unwrap();
        ...
        let cert_ders: Vec<_> = test_certificates
            .iter()
            .map(|cert| {
                let pem_data = cert.certificate_pem.clone().unwrap();
                rustls_pemfile::certs(&mut pem_data.as_bytes())
        ...
        assert!(
            serial_duration > Duration::from_nanos(0),
            "Serial validation should take measurable time"
        );
        assert!(
            batch_duration > Duration::from_nanos(0),
            "Batch validation should take measurable time"
        );

        // 7. Test caching performance
        let start = std::time::Instant::now();
        for cert in &test_certificates[..5] {
            // Test first 5 again (should be cached)
            let pem_data = cert.certificate_pem.clone().unwrap();
            let der_data = rustls_pemfile::certs(&mut pem_data.as_bytes())
                .filter_map(|r| r.ok())
                .next()
        ...
        assert!(
            cached_duration > Duration::from_nanos(0),
            "Cached validation should take measurable time"
        );
    }
    ...
    async fn test_webpki_error_handling_integration() {
        ...
        for (scenario_name, cert_data) in error_scenarios {
            ...
            let chain_validation = security_manager
                .authentication()
                .validate_certificate_chain_with_webpki(&[certificate])
                .await;
            assert!(
                chain_validation.is_err(),
                "WebPKI chain validation should fail for {}",
                scenario_name
            );

            println!(
                "✅ Error handling test passed for scenario: {}",
                scenario_name
            );
        }
        ...
        let wrong_ca = security_manager
            .certificate_manager()
            .generate_root_ca(wrong_ca_params)
            .await
            .unwrap();

        let wrong_cert =
            create_test_client_cert_with_name(&security_manager, &wrong_ca.id, "wrong-ca-client")
                .await;
        ...
        let wrong_ca_validation = security_manager
            .authentication()
            .validate_certificate_comprehensive(&der_data)
            .await;
    ...
    async fn create_test_client_cert(
    ...
    async fn create_test_client_cert_with_name(
        ...
        let cert_request = CertificateRequest {
            subject,
            role: CertMgrRole::Client,
            san_entries: vec![rcgen::SanType::DnsName(format!("{}.test.local", name))],
            validity_days: Some(365),
            key_type: Some(KeyType::Ecdsa),
            key_size: Some(256),
    ...
    async fn test_certificate_authentication_webpki(
        ...
        let certificate = rustls_pki_types::CertificateDer::from(der_data.clone());
        let _principal = security_manager
            .authentication()
    ...
    async fn test_authorization_flow(
        ...
        // Test authorization
        let auth_key = AclKey::new(principal, "integration-test-topic".into(), Permission::Read);

        let _authorization_result = security_manager
            .authorization()
            .check_authorization(&auth_key)
            .await?;
    ...
    async fn test_webpki_specific_features(
        ...
        let certificate = security_manager
            .authentication()
            .parse_certificate_rustls(&der_data)?;

        // Test WebPKI certificate metadata extraction
        let _metadata = security_manager
            .authentication()
            .extract_certificate_metadata(&der_data)
            .await?;
        ...
        let _cert_metadata = security_manager
            .authentication()
            .extract_certificate_metadata(&der_data)
            .await?;
