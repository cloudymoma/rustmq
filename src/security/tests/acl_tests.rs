...
mod tests {
    ...
    impl TestAclManager {
        ...
        async fn store_acl_rules(&self, principal: &str, rules: Vec<AclRule>) -> Result<(), RustMqError> {
            for rule in rules {
        ...
        async fn update_acl_rules(&self, principal: &str, rules: Vec<AclRule>) -> Result<(), RustMqError> {
        ...
        async fn check_permission(&self, principal: &str, topic: &str, operation: AclOperation) -> Result<bool, RustMqError> {
        ...
        async fn bulk_store_acl_rules(&self, rules_map: HashMap<String, Vec<AclRule>>) -> Result<(), RustMqError> {
        ...
        async fn get_recent_audit_entries(&self, _count: usize) -> Result<Vec<String>, RustMqError> {
    ...
    async fn test_acl_rule_storage_and_retrieval() {
        ...
        // Store ACL rules
        let store_result = acl_manager.store_acl_rules(principal, test_rules.clone()).await;
        assert!(store_result.is_ok(), "Storing ACL rules should succeed");
        
        // Retrieve ACL rules
        let retrieved_rules = acl_manager.get_acl_rules(principal).await;
        assert!(retrieved_rules.is_ok(), "Retrieving ACL rules should succeed");
        
        let rules = retrieved_rules.unwrap();
        assert_eq!(rules.len(), test_rules.len(), "Retrieved rules count should match stored count");
        ...
        for stored_rule in &test_rules {
            ...
            assert!(matching_rule.is_some(), "Stored rule should be found in retrieved rules");
    ...
    async fn test_acl_rule_updates_and_versioning() {
        ...
        acl_manager.store_acl_rules(principal, initial_rules.clone()).await.unwrap();
        ...
        acl_manager.update_acl_rules(principal, updated_rules.clone()).await.unwrap();
        
        // Verify version incremented
        let updated_version = acl_manager.get_acl_version().await.unwrap();
        assert!(updated_version > initial_version, "Version should increment after update");
    ...
    async fn test_acl_pattern_matching() {
        ...
        assert!(exact_pattern.matches("topic", "exact-topic"), "Exact pattern should match exactly");
        assert!(!exact_pattern.matches("topic", "different-topic"), "Exact pattern should not match different topic");
        assert!(!exact_pattern.matches("cluster", "exact-topic"), "Exact pattern should not match different resource type");
        
        // Test wildcard patterns
        let wildcard_pattern = ResourcePattern::new(ResourceType::Topic, "*".to_string());
        assert!(wildcard_pattern.matches("topic", "any-topic"), "Wildcard should match any topic");
        assert!(wildcard_pattern.matches("topic", ""), "Wildcard should match empty topic");
        assert!(!wildcard_pattern.matches("cluster", "any-topic"), "Wildcard should not match different resource type");
        
        // Test prefix patterns
        let prefix_pattern = ResourcePattern::new(ResourceType::Topic, "data.*".to_string());
        assert!(prefix_pattern.matches("topic", "data.user-events"), "Prefix pattern should match");
        assert!(prefix_pattern.matches("topic", "data.system-logs"), "Prefix pattern should match");
        assert!(!prefix_pattern.matches("topic", "events.user-data"), "Prefix pattern should not match non-prefixed");
        assert!(!prefix_pattern.matches("topic", "data"), "Prefix pattern should not match exact prefix without dot");
        
        // Test suffix patterns
        let suffix_pattern = ResourcePattern::new(ResourceType::Topic, "*.logs".to_string());
        assert!(suffix_pattern.matches("topic", "application.logs"), "Suffix pattern should match");
        assert!(suffix_pattern.matches("topic", "system.logs"), "Suffix pattern should match");
        assert!(!suffix_pattern.matches("topic", "logs.archive"), "Suffix pattern should not match non-suffixed");
    }
    ...
    async fn test_acl_conflict_resolution() {
        ...
        acl_manager.store_acl_rules(principal, conflicting_rules).await.unwrap();
        
        // Test conflict resolution (deny should take precedence)
        let resolved_permission = acl_manager.check_permission(
            principal,
            "sensitive-topic",
            AclOperation::Read
        ).await;
        
        assert!(resolved_permission.is_ok(), "Permission check should succeed");
        assert!(!resolved_permission.unwrap(), "Deny rule should take precedence over allow rule");
    }
    ...
    async fn test_bulk_acl_operations() {
        ...
        let bulk_rules: HashMap<String, Vec<AclRule>> = (0..100).map(|i| {
        ...
        let individual_rules: HashMap<String, Vec<AclRule>> = (100..110).map(|i| {
        ...
        for (principal, rules) in individual_rules {
            acl_manager.store_acl_rules(&principal, rules).await.unwrap();
        }
        ...
        assert!(bulk_per_operation < Duration::from_secs(1), "Bulk operations should be reasonably fast");
        assert!(individual_per_operation < Duration::from_secs(1), "Individual operations should be reasonably fast");
        
        // Verify all bulk-stored rules can be retrieved
        for (principal, expected_rules) in bulk_rules {
            let retrieved_rules = acl_manager.get_acl_rules(&principal).await.unwrap();
            assert_eq!(retrieved_rules.len(), expected_rules.len(), "Bulk stored rules should be retrievable");
        }
    }
    ...
    async fn test_acl_audit_trail() {
        ...
        acl_manager.store_acl_rules(principal, test_rules.clone()).await.unwrap();
        ...
        acl_manager.update_acl_rules(principal, updated_rules).await.unwrap();
        acl_manager.delete_acl_rules(principal).await.unwrap();
        
        // Verify audit trail
        let final_audit_count = acl_manager.get_audit_log_count().await.unwrap();
        assert!(final_audit_count > initial_audit_count, "Audit log should record ACL operations");
    ...
    async fn test_acl_backup_and_recovery() {
        ...
        // Store test data
        let test_data: HashMap<String, Vec<AclRule>> = (0..20).map(|i| {
        ...
        for (principal, rules) in &test_data {
            acl_manager.store_acl_rules(principal, rules.clone()).await.unwrap();
        }
        ...
        for principal in test_data.keys() {
            ...
            assert!(rules.is_err() || rules.unwrap().is_empty(), "Data should be cleared");
        }
    ...
    async fn test_acl_synchronization_to_brokers() {
        ...
        assert!(bulk_sync_result.is_ok(), "Bulk ACL synchronization should succeed");
        ...
        assert!(mixed_sync_result.is_ok(), "Should handle mixed sync results");
    }
