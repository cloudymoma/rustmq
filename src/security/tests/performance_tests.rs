...
mod tests {
    ...
    use crate::security::acl::{AclManager, AclOperation, AclRule};
    ...
    #[cfg_attr(
        debug_assertions,
        ignore = "Performance tests only run in release mode"
    )]
    async fn test_authorization_latency_requirements() {
        ...
        let test_keys: Vec<AclKey> = (0..10_000)
            .map(|i| {
                AclKey::new(
        ...
        for i in 0..iterations {
            ...
            let _result = security_manager.authorization().get_from_l2_cache(key);
        }
        ...
        assert!(
            avg_latency < Duration::from_nanos(5000),
            "Average L2 cache latency {}ns exceeds 5000ns requirement",
            avg_latency.as_nanos()
        );

        println!(
            "L2 Cache Performance: {} operations in {}ms, avg latency: {}ns",
            iterations,
        ...
        let connection_cache = security_manager.authorization().create_connection_cache();
        ...
        assert!(
            l1_avg_latency < Duration::from_nanos(1200),
            "Average L1 cache latency {}ns exceeds 1200ns requirement",
            l1_avg_latency.as_nanos()
        );

        println!(
            "L1 Cache Performance: {} operations in {}ms, avg latency: {}ns",
            iterations,
    ...
    async fn test_certificate_validation_performance() {
        ...
        for i in 0..iterations {
            ...
            let _result = security_manager
                .authentication()
                .validate_certificate(&cert.certificate_pem.clone().unwrap().as_bytes())
                .await;
        }
        ...
        assert!(
            avg_validation_time < Duration::from_millis(1),
            "Average certificate validation time {}μs exceeds 1ms requirement",
            avg_validation_time.as_micros()
        );

        println!(
            "Certificate Validation Performance: {} validations in {}ms, avg: {}μs",
            iterations,
        ...
        assert!(
            avg_extraction_time < Duration::from_micros(500),
            "Average principal extraction time {}μs exceeds 500μs requirement",
            avg_extraction_time.as_micros()
        );

        println!(
            "Principal Extraction Performance: {} extractions in {}ms, avg: {}μs",
            iterations,
    ...
    async fn test_memory_usage_and_string_interning() {
        ...
        let unique_principal_count = security_manager.authorization().get_interned_string_count();

        assert!(
            unique_principal_count <= repeated_principals.len() + 100, // +100 for topics
            "Interned string count {} should be close to unique count {}",
            unique_principal_count,
            repeated_principals.len()
        );

        // Test that repeated strings share the same Arc<str>
        let first_principal = &interned_principals[0];
        let same_principal = security_manager
            .authorization()
            .intern_string(first_principal.as_ref());

        assert!(
            Arc::ptr_eq(first_principal, &same_principal),
            "Repeated string interning should return the same Arc<str>"
        );

        println!(
            "String Interning Efficiency: {} total strings, {} unique interned",
            acl_keys.len(),
            unique_principal_count
        );
        ...
        let memory_savings_percent =
            (1.0 - (with_interning_estimate as f64 / without_interning_estimate as f64)) * 100.0;
        ...
        println!(
            "Estimated memory savings from string interning: {:.1}%",
            memory_savings_percent
        );
    }
    ...
    async fn test_concurrent_security_operations_stress() {
                ...
                for i in 0..operations_per_task {
                    ...
                    match operation_type {
                        ...
                        1 => {
                            ...
                            let _interned =
                                security_manager.authorization().intern_string(&principal);
                        ...
                        2 => {
                            ...
                            let _status = security_manager
                                .certificate_manager()
                                .get_certificate_status(&cert_id)
                                .await;
        ...
        println!(
            ...
            all_metrics.get("authz_operations").unwrap_or(&0)
        );
        println!(
            "  Authentication operations: {}",
            all_metrics.get("auth_operations").unwrap_or(&0)
        );
        println!(
            "  Certificate operations: {}",
            all_metrics.get("cert_operations").unwrap_or(&0)
        );

        // Verify system is still healthy after stress test
        let final_metrics = security_manager.metrics();
        assert!(
            final_metrics.snapshot().authz_success_count > 0,
    ...
    async fn test_bloom_filter_performance_and_accuracy() {
        ...
        for key in keys_in_filter {
            security_manager.authorization().add_to_bloom_filter(key);
        }
        ...
        for i in 0..iterations {
            let key = &test_keys[i % test_keys.len()];
            let _contains = security_manager.authorization().bloom_filter_contains(key);
        }
        ...
        assert!(
            avg_bloom_latency < Duration::from_nanos(2000),
            "Average bloom filter latency {}ns exceeds 2000ns requirement",
            avg_bloom_latency.as_nanos()
        );

        println!(
            "Bloom Filter Performance: {} lookups in {}ms, avg latency: {}ns",
            iterations,
            bloom_duration.as_millis(),
            avg_bloom_latency.as_nanos()
        );
        ...
        for key in keys_in_filter.iter().take(1000) {
            // Sample for testing
        ...
        for key in keys_not_in_filter.iter().take(1000) {
            // Sample for testing
            if security_manager.authorization().bloom_filter_contains(key) {
                false_positives += 1;
            } else {
    ...
    async fn test_cache_performance_under_load() {
        ...
        let test_keys: Vec<AclKey> = (0..dataset_size)
            .map(|i| {
                AclKey::new(
        ...
        for i in 0..iterations {
            let key = if i % 5 < 4 {
                // 80% of accesses to hot keys
                &hot_keys[i % hot_keys.len()]
            } else {
                // 20% of accesses to cold keys
        ...
        assert!(
            avg_access_time < Duration::from_nanos(10000),
        ...
        assert!(
            hit_rate > 0.15, // At least 15% hit rate expected
