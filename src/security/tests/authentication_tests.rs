...
mod tests {
    ...
    async fn test_certificate_validation_with_valid_cert() {
        ...
        assert!(
            !ca_chain_before.is_empty(),
            "CA chain should not be empty after generating root CA"
        );
    ...
    async fn test_principal_extraction_from_certificate() {
        ...
        subject.push(
            ...
            "client-principal-001".to_string(),
        );
        subject.push(
        ...
        assert!(
            principal.contains("client-principal-001"),
    ...
    async fn test_certificate_caching_behavior() {
        ...
        assert!(
            first_duration >= Duration::from_nanos(0),
            "First duration should be measured"
        );
        assert!(
            second_duration >= Duration::from_nanos(0),
            "Second duration should be measured"
        );
    }
    ...
    async fn test_authentication_context_creation() {
        ...
        subject.push(
            rcgen::DnType::CommonName,
            "broker-001.cluster.internal".to_string(),
        );
        subject.push(
            rcgen::DnType::OrganizationName,
            "RustMQ Cluster".to_string(),
        );
    ...
    async fn test_authentication_metrics_collection() {
        ...
        assert!(
            !ca_chain.is_empty(),
            "CA chain should not be empty after refresh"
        );
        ...
        assert!(
            final_metrics.authentication_success_count > initial_success_count,
            "Success count should increase"
        );
        assert!(
            final_metrics.authentication_failure_count > initial_failure_count,
    ...
    async fn test_certificate_chain_validation() {
        ...
        let root_der = rustls_pemfile::certs(&mut root_pem.as_bytes())
            .filter_map(|r| r.ok())
            .next()
            .unwrap();
        ...
        let chain_validation = auth_manager
            .validate_certificate_chain(&[client_cert_obj, root_ca_obj])
            .await;
    ...
    async fn test_concurrent_authentication_operations() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                let revocation_result = auth_manager
                    .is_certificate_revoked(&client_cert.fingerprint)
                    .await;
                let pem_data = client_cert.certificate_pem.clone().unwrap();
                let certificate = rustls_pemfile::certs(&mut pem_data.as_bytes())
        ...
        for handle in handles {
            ...
            assert!(
                revocation.is_ok(),
                "Concurrent revocation check should succeed"
            );
            assert!(
                principal.is_ok(),
                "Concurrent principal extraction should succeed"
            );
        }
    }
    ...
    async fn test_authentication_with_expired_certificate() {
        ...
        assert!(
            ...
            "Validation of fresh certificate should succeed: {:?}",
            validation_result
        );
    ...
    async fn test_authentication_error_handling() {
        ...
        for (cert_data, description) in test_cases {
            ...
            assert!(
                ...
                "Principal extraction should fail for {}",
                description
            );
        }
    }
    ...
    async fn test_certificate_injection_attacks() {
        ...
        for malicious_cert in malicious_certificates {
            ...
            assert!(
                principal_result.is_err(),
                "Should reject malicious certificate for principal extraction"
            );
        }
    }
    ...
    async fn test_certificate_timing_attack_resistance() {
        ...
        let valid_cert_der = rustls_pemfile::certs(&mut pem_data.as_bytes())
            .filter_map(|r| r.ok())
            .next()
            .unwrap();
        ...
        let avg_valid_time: Duration =
            valid_times.iter().sum::<Duration>() / valid_times.len() as u32;
        ...
        if max_valid_time > min_time_nanos {
            ...
            println!(
                "Skipping timing attack test - operations too fast to measure reliably: valid={:?}, invalid={:?}",
    ...
    async fn test_certificate_chain_manipulation_attacks() {
        ...
        let manipulation_attacks = vec![
            ...
            vec![CertificateDer::from(b"malicious certificate".to_vec())],
        ...
        for (i, malicious_chain) in manipulation_attacks.iter().enumerate() {
            ...
            let result = auth_manager
                .validate_certificate_chain(malicious_chain)
                .await;
            ...
            if result.is_ok() {
                ...
                assert!(
                    malicious_chain.is_empty(),
                    "Unexpected success for chain manipulation attack {}",
                    i
                );
            }
        }
    ...
    async fn test_concurrent_authentication_safety() {
        ...
        for i in 0..5 {
            ...
            let cert_der = rustls_pemfile::certs(&mut pem_data.as_bytes())
                .filter_map(|r| r.ok())
                .next()
                .unwrap();
        ...
        let test_validation = auth_manager
            .validate_certificate(&test_certificates[0])
            .await;
        assert!(
            test_validation.is_ok(),
        ...
        for i in 0..20 {
            ...
            let handle = tokio::spawn(async move {
                ...
                if i % 3 == 0 {
                    ...
                    // Invalid certificate
                    auth_manager
                        .validate_certificate(b"invalid certificate")
                        .await
                } else {
                    ...
                    auth_manager
                        .validate_certificate(&CertificateDer::from(corrupted))
                        .await
                }
            });
    ...
    async fn test_certificate_format_edge_cases() {
        ...
        for (i, edge_case_cert) in edge_case_certificates.iter().enumerate() {
            ...
            assert!(
                principal_result.is_err(),
                "Principal extraction should fail for edge case {}",
                i
            );
        }
    }
    ...
    async fn test_certificate_revocation_bypass_attempts() {
        ...
        subject.push(
            rcgen::DnType::CommonName,
            "revocation-test-user".to_string(),
        );
        ...
        // Revoke the certificate
        cert_manager
            .revoke_certificate(&cert_result.id, RevocationReason::KeyCompromise)
            .await
            .unwrap();
        auth_manager.refresh_revoked_certificates().await.unwrap();
        ...
        if let Err(e) = &result1 {
            assert!(
                ...
                "Should fail with revocation error, got: {}",
                e
            );
        }
        ...
        let result2 = auth_manager
            .validate_certificate(&CertificateDer::from(modified_cert))
            .await;
        assert!(
            result2.is_err(),
            "Modified certificate should fail validation"
        );
        ...
        for _ in 0..10 {
            ...
            assert!(
                result.is_err(),
                "Revoked certificate should consistently be rejected on replay"
            );
        }
        ...
        let legitimate_cert = cert_manager
            .issue_certificate(legitimate_request)
            .await
            .unwrap();
        ...
        let legitimate_der = rustls_pemfile::certs(&mut legitimate_pem.as_bytes())
            .filter_map(|r| r.ok())
            .next()
            .unwrap();
    ...
    async fn test_principal_extraction_security() {
        ...
        for (i, malicious_subject) in malicious_subjects.iter().enumerate() {
            ...
            let fake_certificate = CertificateDer::from(
                format!(
                    "Subject: {}\nIssuer: Test CA\nSerial: {}",
                    malicious_subject, i
                )
            ...
            let result =
                auth_manager.extract_principal_from_certificate(&fake_certificate, &fingerprint);
            ...
            match result {
                Ok(principal) => {
                    ...
                    assert!(
                        !principal.contains("DROP TABLE"),
                        "Principal should not contain SQL injection"
                    );
                    assert!(
                        !principal.contains("<script>"),
                        "Principal should not contain script injection"
                    );
                    assert!(
                        !principal.contains("../"),
                        "Principal should not contain path traversal"
                    );
                    assert!(
                        !principal.contains("\\..\\"),
                        "Principal should not contain Windows path traversal"
                    );
                    assert!(
                        !principal.contains('\x00'),
                        "Principal should not contain null bytes"
                    );
                }
                Err(_) => {
