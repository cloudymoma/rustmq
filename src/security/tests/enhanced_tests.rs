...
use crate::security::auth::certificate_metadata::{
    CertificateMetadata, CertificateRole, KeyStrength,
};
use crate::security::metrics::SecurityMetrics;
use crate::security::tests::SecurityTestConfig;
use crate::security::tls::{CertificateInfo, CertificateManager};
...
async fn test_enhanced_certificate_metadata_extraction() {
    ...
    match metadata_result {
        Ok(metadata) => {
            ...
            assert!(
                !metadata.subject_dn.raw_dn.is_empty(),
                "Subject DN should not be empty"
            );
            assert!(
                !metadata.issuer_dn.raw_dn.is_empty(),
                "Issuer DN should not be empty"
            );
...
async fn test_enhanced_validation_config() {
    ...
    assert!(
        ...
        "Should require strong keys by default"
    );
    assert!(
        config.max_validity_days.is_some(),
        "Should have max validity days"
    );
    assert!(!config.require_san, "Should not require SAN by default");
    assert_eq!(
        config.expiry_warning_days, 30,
        "Should warn 30 days before expiry"
    );
    assert!(
        !config.allowed_roles.is_empty(),
        "Should have allowed roles"
    );

    // Test minimum key sizes
    assert!(
        config.min_key_sizes.contains_key("RSA"),
        "Should have RSA minimum"
    );
    assert!(
        config.min_key_sizes.contains_key("ECDSA"),
        "Should have ECDSA minimum"
    );
...
async fn test_enhanced_key_strength_classification() {
    ...
    for strength in strengths {
        ...
        assert_eq!(
            ...
            "Key strength should be copyable and comparable"
        );
    }
...
async fn test_enhanced_cache_statistics() {
    ...
    assert_eq!(
        stats.metadata_cache_size, 0,
        "Initial metadata cache should be empty"
    );
    assert_eq!(
        stats.parsed_cert_cache_size, 0,
        "Initial parsed cert cache should be empty"
    );
    assert_eq!(
        stats.principal_cache_size, 0,
        "Initial principal cache should be empty"
    );
    assert_eq!(
        stats.total_cache_entries, 0,
        "Initial total cache should be empty"
    );
...
async fn test_enhanced_cache_cleanup() {
    ...
    assert_eq!(
        stats.total_cache_entries, 0,
        "Cache should remain empty after cleanup"
    );
...
fn create_simple_test_cert_der() -> Vec<u8> {
    ...
    vec![
        ...
        0x00, // SHA256WithRSAEncryption
        0x30, 0x12, // Issuer
        0x31, 0x10, 0x30, 0x0E, 0x06, 0x03, 0x55, 0x04, 0x03, 0x0C, 0x07, 0x54, 0x65, 0x73, 0x74,
        0x20, 0x43, 0x41, // "Test CA"
        0x30, 0x1E, // Validity
        ...
        0x5A, // NotAfter
        0x30, 0x15, // Subject
        0x31, 0x13, 0x30, 0x11, 0x06, 0x03, 0x55, 0x04, 0x03, 0x0C, 0x0A, 0x54, 0x65, 0x73, 0x74,
        0x20, 0x43, 0x6C, 0x69, 0x65, 0x6E,
        0x74, // "Test Client"
