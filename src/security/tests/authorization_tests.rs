...
mod tests {
    ...
    async fn test_l1_cache_operations() {
        ...
        assert!(
            connection_cache.get(&acl_key).is_none(),
        ...
        for i in 0..1100 {
            // Exceed capacity of 1000
            let key = AclKey::new(
                format!("principal-{}", i).into(),
                "topic".into(),
                Permission::Read,
            );
        ...
        assert!(
            cache_size <= 1000,
            "Cache should not exceed capacity of 1000"
        );
    }
    ...
    async fn test_l2_cache_operations() {
        ...
        // Initially should be empty
        assert!(
            auth_manager.get_from_l2_cache(&acl_key).is_none(),
            "L2 cache should be empty initially"
        );
    ...
    async fn test_bloom_filter_negative_caching() {
        ...
        assert!(
            !auth_manager.bloom_filter_contains(&unknown_key),
            "Bloom filter should not contain unknown key"
        );
        ...
        assert!(
            auth_manager.bloom_filter_contains(&unknown_key),
            "Bloom filter should contain added key"
        );
        ...
        assert!(
            ...
            "Bloom filter lookup should be very fast"
        );
    }
    ...
    async fn test_string_interning_memory_efficiency() {
        ...
        assert!(
            unique_count <= 12,
            "Should have at most 12 unique interned strings, got {}",
            unique_count
        );
    }
    ...
    async fn test_cache_hit_miss_statistics() {
        ...
        let test_key = AclKey::new(
            "test-principal".to_string().into(),
            "test-topic".to_string().into(),
            Permission::Read,
        );
    ...
    async fn test_authorization_performance_requirements() {
        ...
        let test_keys: Vec<AclKey> = (0..1000)
            .map(|i| {
                AclKey::new(
        ...
        assert!(
            avg_latency < Duration::from_nanos(max_latency_ns),
    ...
    async fn test_batch_acl_fetching() {
        ...
        let batch_keys: Vec<AclKey> = (0..20)
            .map(|i| {
                AclKey::new(
        ...
        assert!(
            ...
            "Batch duration should be measured"
        );
        assert!(
            individual_duration > Duration::from_nanos(0),
            "Individual duration should be measured"
        );
    }
    ...
    async fn test_cache_invalidation() {
        ...
        let keys_to_invalidate: Vec<AclKey> = (0..10)
            .map(|i| {
                AclKey::new(
        ...
        for key in &keys_to_invalidate {
            assert!(
                auth_manager.get_from_l2_cache(key).is_some(),
        ...
        auth_manager
            .invalidate_principal("invalidate-principal-5")
            .await
            .unwrap();
        ...
        assert!(
            auth_manager.get_from_l2_cache(invalidated_key).is_none(),
            "Invalidated entry should be removed"
        );

        // Other entries should still be present
        let other_key = &keys_to_invalidate[3];
        assert!(
            auth_manager.get_from_l2_cache(other_key).is_some(),
        ...
        for key in &keys_to_invalidate {
            assert!(
                auth_manager.get_from_l2_cache(key).is_none(),
    ...
    async fn test_cache_warming_and_preloading() {
        ...
        let avg_access_time =
            access_duration / (frequent_principals.len() * common_topics.len()) as u32;
    ...
    async fn test_authorization_metrics_collection() {
        ...
        assert!(
            final_metrics.l1_cache_hits >= initial_l1_hits,
            "L1 hits should not decrease"
        );
        assert!(
            final_metrics.l2_cache_hits >= initial_l2_hits,
            "L2 hits should not decrease"
        );
        assert!(
            final_metrics.l2_cache_misses >= initial_misses,
    ...
    async fn test_malicious_topic_patterns() {
        ...
        for malicious_topic in malicious_topics {
            ...
            assert!(
                result.is_ok() || result.is_err(),
                "Should handle malicious topic patterns gracefully"
            );
        }
    }
    ...
    async fn test_cache_poisoning_resistance() {
        ...
        assert_eq!(
            connection_cache.get(&legitimate_key),
            Some(true),
            "Legitimate entry should remain"
        );
        ...
        assert_eq!(
            connection_cache.get(&malicious_key),
            Some(true),
            "Cache should store entries separately"
        );
    }
    ...
    async fn test_concurrent_cache_race_conditions() {
        ...
        assert!(
            ...
            "Cache should be in consistent state after concurrent access"
        );
    }
    ...
    async fn test_cache_memory_exhaustion_protection() {
        ...
        assert!(
            ...
            "Insert should remain fast"
        );
        assert!(
            lookup_time < Duration::from_millis(1),
            "Lookup should remain fast"
        );
    ...
    async fn test_string_interning_memory_safety() {
        ...
        for malicious_string in malicious_strings {
            ...
            assert!(
                !interned.is_empty() || malicious_string.is_empty(),
                "Interning should preserve non-empty strings"
            );
        }
        ...
        for interned in &interned_strings[1..] {
            assert_eq!(
                Arc::as_ptr(interned),
                first_ptr,
                "Repeated strings should share the same Arc instance"
            );
        }
    }
    ...
    async fn test_permission_escalation_prevention() {
        ...
        for permission in permissions {
            let result = auth_manager
                .check_permission(&connection_cache, &test_principal, test_topic, permission)
                .await;
        ...
        assert!(
            final_check.is_ok(),
            "System should remain responsive after many checks"
        );
    }
    ...
    async fn test_bloom_filter_false_positive_handling() {
        ...
        for key in &test_keys {
            assert!(
                auth_manager.bloom_filter_contains(key),
