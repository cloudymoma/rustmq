...
use crate::security::tls::{CertificateManager, CaGenerationParams, KeyType};
...
async fn test_enhanced_validation_policy_configuration() {
    ...
    assert!(!config.require_strong_keys, "Strong keys requirement should be disabled");
    assert_eq!(config.max_validity_days.unwrap(), 180, "Max validity should be 6 months");
    assert!(config.require_san, "SAN requirement should be enabled");
    assert_eq!(config.expiry_warning_days, 15, "Expiry warning should be 15 days");
...
async fn test_enhanced_comprehensive_validation_result() {
    ...
    match validation_result {
        Ok(result) => {
            ...
            assert!(result.validation_duration.as_nanos() > 0, "Validation should take some time");
...
async fn test_enhanced_certificate_purpose_validation() {
    ...
    for purpose in purposes {
        let validation_result = auth_manager.validate_certificate_for_purpose(&cert_der, purpose).await;
        ...
        match validation_result {
            ...
            Err(e) => {
                println!("⚠️  Certificate validation failed for purpose {:?}: {}", purpose, e);
...
async fn test_enhanced_batch_certificate_validation() {
    ...
    assert_eq!(results.len(), 3, "Should get results for all 3 certificates");
...
async fn test_enhanced_cache_monitoring() {
    ...
    assert_eq!(initial_stats.total_cache_entries, 0, "Initial cache should be empty");
    ...
    println!("   - Initial total entries: {}", initial_stats.total_cache_entries);
    println!("   - Updated total entries: {}", updated_stats.total_cache_entries);
    println!("   - Metadata cache: {}", updated_stats.metadata_cache_size);
    println!("   - Validation cache: {}", updated_stats.validation_cache_size);
...
fn create_minimal_test_cert() -> Vec<u8> {
    ...
    vec![
        ...
        0x0C, 0x14, 0x52, 0x75, 0x73, 0x74, 0x4D, 0x51, 0x20, 0x54, 0x65, 0x73, 0x74, 0x20, 0x52, 0x6F, 0x6F, 0x74, 0x20, 0x43, 0x41, // "RustMQ Test Root CA"
        ...
        0x17, 0x0D, // UTCTime tag
        0x32, 0x34, 0x31, 0x32, 0x33, 0x31, 0x32, 0x33, 0x35, 0x39, 0x35, 0x39, 0x5A, // "241231235959Z"
        0x30, 0x22, // Subject SEQUENCE (34 bytes)
        0x31, 0x20, 0x30, 0x1E, 0x06, 0x03, 0x55, 0x04, 0x03, // CN attribute
        0x0C, 0x17, 0x52, 0x75, 0x73, 0x74, 0x4D, 0x51, 0x20, 0x54, 0x65, 0x73, 0x74, 0x20, 0x43, 0x6C, 0x69, 0x65, 0x6E, 0x74, 0x20, 0x43, 0x65, 0x72, 0x74, // "RustMQ Test Client Cert"
