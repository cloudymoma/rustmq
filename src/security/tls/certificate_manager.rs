...
impl CertificateCacheKey {
    ...
    fn extract_subject_key_id(cert_der: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        let (_, cert) =
            X509Certificate::from_der(cert_der).map_err(|_| RustMqError::InvalidCertificate {
...
impl CertificateManager {
    ...
    pub async fn new_with_enhanced_config(
        ...
        let storage = Arc::new(LocalObjectStorage::new(PathBuf::from(
            &config.storage_path,
    ...
    pub async fn generate_root_ca(&self, params: CaGenerationParams) -> Result<CertificateInfo> {
        ...
        let key_pair = self.generate_key_pair(
            params.key_type.unwrap_or(self.config.ca_settings.key_type),
        ...
        if let Some(org) = params
            .organization
            .or_else(|| Some(self.config.ca_settings.organization.clone()))
        {
            distinguished_name.push(rcgen::DnType::OrganizationName, org);
        }

        if let Some(ou) = params
            .organizational_unit
            .or_else(|| Some(self.config.ca_settings.organizational_unit.clone()))
        {
            distinguished_name.push(rcgen::DnType::OrganizationalUnitName, ou);
        }

        if let Some(country) = params
            .country
            .or_else(|| Some(self.config.ca_settings.country_code.clone()))
        {
        ...
        let validity_years = params
            .validity_years
            .unwrap_or(self.config.ca_settings.root_ca_validity_years);
        // Set certificate validity period
        cert_params.not_before = ::time::OffsetDateTime::now_utc();
        cert_params.not_after =
            cert_params.not_before + ::time::Duration::days(validity_years as i64 * 365);

        cert_params.key_pair = Some(key_pair);

        let certificate = RcgenCertificate::from_params(cert_params).map_err(|e| {
        ...
        let cert_info = self
            .store_certificate(certificate, CertificateRole::RootCa, None, "system")
            .await?;
    ...
    pub async fn issue_certificate(&self, request: CertificateRequest) -> Result<CertificateInfo> {
        ...
        let template = self
            ...
            .get(&request.role)
            .ok_or_else(|| {
                RustMqError::Config(format!("No template found for role: {:?}", request.role))
            })?;
        ...
        let san_strings: Vec<String> = request
            .san_entries
            .iter()
            .map(|san| {
                match san {
        ...
        cert_params.key_usages = template
            .key_usage
            .iter()
            .map(|ku| match ku {
        ...
        cert_params.extended_key_usages = template
            .extended_key_usage
            .iter()
            .map(|eku| match eku {
        ...
        cert_params.not_before = ::time::OffsetDateTime::now_utc();
        cert_params.not_after =
            cert_params.not_before + ::time::Duration::days(validity_days as i64);
        ...
        let cert_info = if let Some(ref issuer_id) = request.issuer_id {
            ...
            let signed_der = certificate
                .serialize_der_with_signer(&issuer_cert)
            ...
            self.store_certificate(certificate, request.role, None, "user")
                .await?
        };
    ...
    pub async fn renew_certificate(&self, cert_id: &str) -> Result<CertificateInfo> {
        ...
        let existing_cert = self.get_certificate_by_id(cert_id).await?.ok_or_else(|| {
            RustMqError::CertificateNotFound {
        ...
        for part in subject_parts {
            if let Some((key, value)) = part.split_once("=") {
                match key {
                    ...
                    "O" => {
                        distinguished_name.push(rcgen::DnType::OrganizationName, value.to_string())
                    }
                    "OU" => distinguished_name
                        .push(rcgen::DnType::OrganizationalUnitName, value.to_string()),
        ...
        let san_entries: Vec<SanType> = existing_cert
            .san_entries
            .iter()
        ...
        self.revoke_certificate(cert_id, RevocationReason::Superseded)
            .await?;
    ...
    pub async fn revoke_certificate(&self, cert_id: &str, reason: RevocationReason) -> Result<()> {
        ...
        // Update certificate status
        {
            let mut certificates = self.certificates.write().map_err(|_| {
                RustMqError::Storage(
                    "Failed to acquire certificate lock for revocation".to_string(),
                )
            })?;
    ...
    pub async fn get_expiring_certificates(
        ...
        let expiring: Vec<CertificateInfo> = certificates
            .values()
    ...
    pub async fn list_all_certificates(&self) -> Result<Vec<CertificateInfo>> {
        ...
        let all_certs: Vec<CertificateInfo> = certificates
            .values()
    ...
    pub async fn get_ca_chain(&self) -> Result<Vec<CertificateDer<'static>>> {
        ...
        for cert_metadata in certificates.values().filter(|cert_metadata| {
            cert_metadata.info.is_ca && cert_metadata.info.status == CertificateStatus::Active
        }) {
    ...
    async fn reconstruct_rcgen_certificate(&self, cert_id: &str) -> Result<RcgenCertificate> {
        let cert_metadata = {
            let certificates = self.certificates.read().map_err(|_| {
                RustMqError::Storage(
                    "Failed to acquire certificate lock for reconstruction".to_string(),
                )
            })?;
        ...
        let private_key_pem = if let Some(ref pem) = cert_metadata.info.private_key_pem {
            ...
            // Load from storage
            let key_data =
                self.storage.get(key_path).await.map_err(|e| {
                    RustMqError::Storage(format!("Failed to load private key: {}", e))
                })?;
        ...
        let key_pair =
            KeyPair::from_pem(&private_key_pem).map_err(|e| RustMqError::InvalidCertificate {
        ...
        for extension in parsed_cert.extensions() {
            match extension.oid.to_string().as_str() {
                ...
                "2.5.29.37" => {
                    if let x509_parser::extensions::ParsedExtension::ExtendedKeyUsage(usage) =
                        extension.parsed_extension()
                    {
                ...
                "2.5.29.19" => {
                    if let x509_parser::extensions::ParsedExtension::BasicConstraints(constraints) =
                        extension.parsed_extension()
                    {
                        if constraints.ca {
        ...
        RcgenCertificate::from_params(cert_params).map_err(|e| RustMqError::InvalidCertificate {
    ...
    fn generate_key_pair(&self, key_type: KeyType, key_size: u32) -> Result<KeyPair> {
        match key_type {
            KeyType::Rsa => KeyPair::generate(&rcgen::PKCS_RSA_SHA256).map_err(|e| {
                RustMqError::CertificateGeneration {
            ...
            KeyType::Ecdsa => {
                ...
                KeyPair::generate(alg).map_err(|e| RustMqError::CertificateGeneration {
    ...
    async fn store_certificate(
        ...
        let (_, parsed_cert) = X509Certificate::from_der(&der_data).map_err(|e| {
            RustMqError::CertificateGeneration {
        ...
        let not_before =
            UNIX_EPOCH + Duration::from_secs(parsed_cert.validity().not_before.timestamp() as u64);
        let not_after =
            UNIX_EPOCH + Duration::from_secs(parsed_cert.validity().not_after.timestamp() as u64);
        let renewal_threshold = not_after
            - Duration::from_secs(self.config.basic.auto_renew_before_expiry_days as u64 * 86400);

        let san_entries: Vec<String> = parsed_cert
            .extensions()
            .iter()
        ...
        let private_key_path = Some(
            self.store_private_key(&cert_id, &certificate.serialize_private_key_pem())
                .await?,
        );
    ...
    async fn store_signed_certificate(
        ...
        // Store private key securely (always encrypted)
        let private_key_path = Some(self.store_private_key(&cert_id, &private_key_pem).await?);
    ...
    async fn store_private_key(&self, cert_id: &str, private_key_pem: &str) -> Result<String> {
        ...
        info!(
            "Encrypting private key with AES-256-GCM for certificate: {}",
            cert_id
        );
        let key_data = key_encryption::encrypt_private_key(
            private_key_pem,
            &self.config.key_encryption_password,
        )?;

        self.storage
            .put(&key_path, key_data.into())
            .await
            ...
            Ok(data) => {
                let certificate_map: HashMap<String, CertificateMetadata> =
                    serde_json::from_slice(&data).map_err(|e| {
                        RustMqError::Storage(format!("Failed to deserialize certificates: {}", e))
                    })?;

                let mut certificates = self.certificates.write().map_err(|_| {
                    RustMqError::Storage(
                        "Failed to acquire certificate lock for loading".to_string(),
                    )
                })?;
    ...
    async fn persist_certificates(&self) -> Result<()> {
        let (data, count) = {
            let certificates = self.certificates.read().map_err(|_| {
                RustMqError::Storage(
                    "Failed to acquire certificate lock for persistence".to_string(),
                )
            })?;

            let data = serde_json::to_vec(&*certificates).map_err(|e| {
                RustMqError::Storage(format!("Failed to serialize certificates: {}", e))
            })?;
        ...
        self.storage
            .put("certificates/certificate_list.json", data.into())
            .await
    ...
    async fn persist_crl(&self) -> Result<()> {
        ...
        self.storage
            .put("certificates/crl.json", data.into())
            .await
    ...
    async fn start_renewal_task(&mut self) -> Result<()> {
        ...
        let task = tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs(
                config.renewal_check_interval_hours * 3600,
            ));
            ...
            loop {
                ...
                let certs_to_renew = {
                    ...
                    let threshold = SystemTime::now()
                        + Duration::from_secs(
                            config.basic.auto_renew_before_expiry_days as u64 * 86400,
                        );
    ...
    pub async fn get_recent_audit_entries(
        &self,
        limit: usize,
    ) -> Result<Vec<CertificateAuditEntry>> {
    ...
    pub async fn get_revoked_certificate_fingerprints(&self) -> Result<Vec<String>> {
        let certificates = self.certificates.read().map_err(|_| {
            RustMqError::Storage(
                "Failed to acquire certificate lock for revoked fingerprints".to_string(),
            )
        })?;
    ...
    pub async fn validate_certificate_cached(
        &self,
        cert_der: &[u8],
        ...
        let cert =
            EndEntityCert::try_from(cert_der).map_err(|e| RustMqError::InvalidCertificate {
    ...
    pub async fn load_ca_chain_cached(&self, ca_cert_ders: Vec<Vec<u8>>) -> Result<usize> {
        ...
        for ca_der in ca_cert_ders {
            ...
            let _cert = EndEntityCert::try_from(ca_der.as_slice()).map_err(|e| {
    ...
    pub fn invalidate_expired_certificates(&self) -> usize {
        ...
        let cert_invalidated = self
            .cert_cache
            .invalidate_by(|_key, entry| entry.expires_at <= now);

        let ca_invalidated = self
            .ca_cache
            .invalidate_by(|_key, entry| entry.expires_at <= now);
