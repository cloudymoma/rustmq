...
use crate::security::tls::{CertificateInfo, CertificateManager, TlsConfig};
...
use super::certificate_metadata::{
    CertificateMetadata, CertificateRole, DistinguishedName, ExtendedKeyUsage, KeyStrength,
    KeyUsage, PublicKeyInfo, SubjectAltName, ValidityPeriod,
};
use der::Decode;
...
impl ValidationCacheEntry {
    ...
    fn to_result(&self) -> Result<(), RustMqError> {
        self.result
            .clone()
            .map_err(|e| RustMqError::CertificateValidation { reason: e })
    }
}
...
impl AuthenticationManager {
    ...
    pub async fn authenticate_connection(
        ...
        if self.is_certificate_revoked(&cert_fingerprint).await? {
            self.metrics
                .record_authentication_failure("certificate_revoked");
        ...
        self.metrics
            .record_authentication_success(start_time.elapsed());
    ...
    fn extract_client_certificates(
        &self,
        connection: &Connection,
    ) -> Result<Vec<CertificateDer<'static>>, RustMqError> {
    ...
    pub async fn validate_certificate_chain(
        ...
        let result = self
            .validate_certificate_chain_with_webpki(certificates)
            .await;
    ...
    async fn validate_certificate_chain_internal(
        &self,
        certificates: &[CertificateDer<'static>],
    ) -> Result<(), RustMqError> {
        ...
        if elapsed < min_processing_time {
            ...
            let sleep_time =
                min_processing_time - elapsed + std::time::Duration::from_nanos(jitter);
    ...
    async fn perform_certificate_validation(
        ...
        if !signature_valid {
            return Err(RustMqError::CertificateValidation {
                reason: "Certificate signature validation failed - not signed by trusted CA"
                    .to_string(),
            });
        }
    ...
    fn parse_certificate<'a>(
        &self,
        cert_der: &'a [u8],
    ) -> Result<X509Certificate<'a>, RustMqError> {
        ...
        for &byte in cert_der {
            ...
            if byte == 0x00 {
                ...
                if consecutive_null > MAX_CONSECUTIVE_NULLS {
                    return Err(RustMqError::InvalidCertificate {
                        reason:
                            "Certificate contains excessive null byte padding indicating tampering"
                                .to_string(),
                    });
                }
            }
            // Check for excessive consecutive 0xFF bytes (less common but suspicious in large sequences)
                ...
                if consecutive_ff > MAX_CONSECUTIVE_FF {
                    return Err(RustMqError::InvalidCertificate {
                        reason:
                            "Certificate contains excessive 0xFF sequences indicating tampering"
                                .to_string(),
                    });
                }
            } else {
    ...
    fn validate_against_root_ca(
        &self,
        cert: &X509Certificate,
        trusted_ca_chain: &[CertificateDer<'static>],
    ...
    pub async fn extract_certificate_metadata(
        &self,
        certificate_der: &[u8],
    ) -> Result<Arc<CertificateMetadata>, RustMqError> {
        ...
        if let Some(cached_entry) = self.metadata_cache.get(&fingerprint) {
            if !cached_entry.is_expired(std::time::Duration::from_secs(3600)) {
                // 1 hour TTL
    ...
    pub async fn extract_principal_enhanced(
        &self,
        certificate: &CertificateDer<'static>,
    ) -> Result<Arc<str>, RustMqError> {
        let metadata = self
            .extract_certificate_metadata(certificate.as_ref())
            .await?;
    ...
    pub async fn validate_certificate_for_purpose(
        ...
        self.validate_certificate_comprehensive(certificate_der)
            .await?;
        ...
        if !metadata.is_valid_for_purpose(required_purpose) {
            return Err(RustMqError::CertificateValidation {
                reason: format!(
                    "Certificate is not valid for purpose: {:?}",
                    required_purpose
                ),
            });
        }
    ...
    pub async fn validate_certificates_batch(
        &self,
        certificates: &[&[u8]],
    ) -> Vec<Result<(), RustMqError>> {
        // Process certificates in parallel using tokio tasks
        let tasks: Vec<_> = certificates
            .iter()
            .map(|cert_der| {
                let manager = self.clone();
                let cert_der = cert_der.to_vec();
                tokio::spawn(
                    async move { manager.validate_certificate_comprehensive(&cert_der).await },
                )
            })
            .collect();
    ...
    pub async fn get_certificate_info(
        &self,
        certificate_der: &[u8],
    ) -> Result<ExtendedCertificateInfo, RustMqError> {
        let metadata = self.extract_certificate_metadata(certificate_der).await?;
    ...
    pub async fn validate_certificate_with_policies(
        ...
        if let Err(e) = self
            .validate_certificate_comprehensive(certificate_der)
            .await
        {
        ...
        if validation_config.require_strong_keys
            && metadata.public_key_info.strength == KeyStrength::Weak
        {
            validation_result
                .validation_errors
                .push("Weak key strength not allowed".to_string());
            validation_result.is_valid = false;
        }
        ...
        if let Some(max_validity_days) = validation_config.max_validity_days {
            ...
            if validity_days > max_validity_days {
                validation_result.validation_errors.push(format!(
                    "Certificate validity period too long: {} days",
                    validity_days
                ));
                validation_result.is_valid = false;
            }
        ...
        if validation_config.require_san && metadata.subject_alt_names.is_empty() {
            validation_result
                .validation_errors
                .push("Subject Alternative Name required but not present".to_string());
            validation_result.is_valid = false;
        }

        // Expiration warnings
        if metadata.is_nearing_expiration(validation_config.expiry_warning_days) {
            validation_result.validation_warnings.push(format!(
                "Certificate expires in {} days",
                metadata.validity_period.days_until_expiry
            ));
        }
    ...
    pub fn get_cache_statistics(&self) -> CacheStatistics {
        CacheStatistics {
            ...
            total_cache_entries: self.metadata_cache.len()
                + self.parsed_cert_cache.len()
                + self.principal_cache.len(),
        }
    }
    ...
    fn validate_certificate_with_webpki(
        ...
        for (i, ca_cert) in ca_certs.iter().enumerate() {
            match TrustAnchor::try_from_cert_der(ca_cert.as_ref()) {
                Ok(trust_anchor) => {
                    ...
                    tracing::debug!(
                        "Successfully converted CA certificate {} to trust anchor",
                        i
                    );
                }
                Err(e) => {
                    let error_msg = format!(
                        "Failed to convert CA certificate {} to trust anchor: {:?}",
                        i, e
                    );
        ...
        if trust_anchors.is_empty() {
            tracing::warn!(
                "WebPKI trust anchor conversion failed, falling back to legacy validation. Errors: {}",
        ...
        tracing::debug!(
            "Successfully converted {}/{} CA certificates to trust anchors",
            trust_anchors.len(),
            ca_certs.len()
        );
        ...
        let end_entity = match EndEntityCert::try_from(cert.as_ref()) {
            ...
            Err(e) => {
                ...
                tracing::warn!(
                    "WebPKI end entity certificate parsing failed: {:?}, falling back to legacy validation",
                    e
                );
                return self.validate_certificate_signature_legacy(cert, ca_certs);
            }
            ...
            if format!("{:?}", server_err).contains("RequiredEkuNotFound") {
                ...
                if let Err(ref client_err) = client_validation_result {
                    if format!("{:?}", client_err).contains("RequiredEkuNotFound") {
                        ...
                        tracing::debug!(
                            "Certificate lacks EKU extensions but is otherwise valid - accepting for simplified root CA architecture"
                        );
                        return Ok(());
                    }
                }
                ...
                match client_validation_result {
                    ...
                    Err(e) => {
                        ...
                        if error_debug.contains("RequiredEkuNotFound") {
                            ...
                            tracing::debug!(
                                "WebPKI client validation failed due to EKU compatibility, attempting legacy validation: {:?}",
                                e
                            );
                            return self.validate_certificate_signature_legacy(cert, ca_certs);
                        } else {
                            ...
                            tracing::error!(
                                "WebPKI client validation failed with structural error (no fallback): {:?}",
                                e
                            );
                            return Err(RustMqError::CertificateValidation {
                ...
                if server_error_debug.contains("RequiredEkuNotFound") {
                    // EKU compatibility issue - allow fallback
                    tracing::debug!(
                        "WebPKI server validation failed due to EKU compatibility, attempting legacy validation: {:?}",
                        server_err
                    );
                    return self.validate_certificate_signature_legacy(cert, ca_certs);
                } else {
                    ...
                    tracing::error!(
                        "WebPKI server validation failed with structural error (no fallback): {:?}",
                        server_err
                    );
                    return Err(RustMqError::CertificateValidation {
    ...
    fn validate_certificate_signature_legacy(
        ...
        tracing::debug!(
            "Legacy validation: checking against {} CA certificates",
            ca_certs.len()
        );
        ...
        for ca_cert in ca_certs {
            if self
                .validate_certificate_signature(&parsed_cert, ca_cert)
                .is_ok()
            {
    ...
    pub fn parse_certificate_rustls(
        &self,
        cert_der: &[u8],
        ...
        EndEntityCert::try_from(certificate.as_ref()).map_err(|e| {
    ...
    pub async fn validate_certificate_chain_with_webpki(
        ...
        match self.validate_certificate_with_webpki(client_cert, &ca_chain) {
            ...
            Err(e) => {
                tracing::warn!(
                    "WebPKI validation failed: {:?}, attempting legacy validation",
                    e
                );
    ...
    pub fn extract_principal_with_webpki(
        ...
        let end_entity = EndEntityCert::try_from(certificate.as_ref()).map_err(|e| {
            RustMqError::InvalidCertificate {
        ...
        match self.validate_certificate_with_webpki(&certificate, &ca_chain) {
            ...
            Err(e) => {
                ...
                let is_corrupted = match webpki::EndEntityCert::try_from(certificate.as_ref()) {
                    Ok(_) => {
                        tracing::debug!(
                            "WebPKI can parse certificate structure - treating as compatibility issue"
                        );
                ...
                if is_corrupted {
                    ...
                    tracing::error!(
                        "Certificate is corrupted (structural parsing failure), rejecting: {:?}",
                        e
                    );
                    return Err(RustMqError::CertificateValidation {
                    ...
                    tracing::debug!(
                        "WebPKI validation failed but certificate structure is valid, attempting legacy validation: {:?}",
                        e
                    );
    ...
    fn validate_certificate_signature(
        ...
        let verification_result = match sig_oid.as_str() {
            // RSA with SHA256
            "1.2.840.113549.1.1.11" => signature::UnparsedPublicKey::new(
                &signature::RSA_PKCS1_2048_8192_SHA256,
                ca_public_key_bytes,
            )
            .verify(tbs_cert, signature_bytes),
            // RSA with SHA384
            "1.2.840.113549.1.1.12" => signature::UnparsedPublicKey::new(
                &signature::RSA_PKCS1_2048_8192_SHA384,
                ca_public_key_bytes,
            )
            .verify(tbs_cert, signature_bytes),
            // RSA with SHA512
            "1.2.840.113549.1.1.13" => signature::UnparsedPublicKey::new(
            ...
            // ECDSA with SHA384
            "1.2.840.10045.4.3.3" => signature::UnparsedPublicKey::new(
                &signature::ECDSA_P384_SHA384_ASN1,
                ca_public_key_bytes,
            )
            .verify(tbs_cert, signature_bytes),
        ...
        match verification_result {
            Ok(()) => {
                tracing::debug!(
                    "Certificate signature verified successfully using algorithm: {}",
                    sig_oid
                );
                Ok(())
            }
            Err(e) => {
                ...
                self.metrics
                    .record_authentication_failure("certificate_signature_invalid");
                Err(RustMqError::InvalidCertificate {
                    reason: format!(
                        "Certificate signature verification failed: Invalid signature - certificate may be forged or corrupted"
                    ),
                })
            }
        }
    ...
    fn parse_certificate_lenient<'a>(
        &self,
        cert_der: &'a [u8],
    ) -> Result<X509Certificate<'a>, RustMqError> {
    ...
    pub fn extract_principal_from_certificate(
        ...
        match self.extract_principal_with_webpki(certificate, fingerprint) {
            ...
            Err(_) => {
                ...
                tracing::debug!(
                    "WebPKI principal extraction failed, falling back to legacy method"
                );
    ...
    pub async fn refresh_revoked_certificates(&self) -> Result<(), RustMqError> {
        ...
        let revoked_fingerprints = self
            .certificate_manager
            .get_revoked_certificate_fingerprints()
            .await?;
    ...
    fn update_metrics_for_result(
        &self,
        result: &Result<(), RustMqError>,
        ...
        if result.is_ok() {
            ...
            self.failure_count
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            self.metrics
                .record_authentication_failure("validation_failed");
        }
    }
    ...
    async fn cache_validation_result(
        ...
        let cached_result = ValidationCacheEntry::new(result.as_ref().map(|_| ()).map_err(|e| {
    ...
    async fn validate_with_parsed_data(
        &self,
        parsed_data: &ParsedCertData,
        fingerprint: &CertificateFingerprint,
    ) -> Result<(), RustMqError> {
    ...
    async fn validate_certificate_optimized(
        &self,
        certificate_der: &[u8],
        fingerprint: &CertificateFingerprint,
    ) -> Result<(), RustMqError> {
        ...
        self.parsed_cert_cache
            .insert(*fingerprint, parsed_data.clone());

        // Use the cached data for validation
        self.validate_with_parsed_data(&parsed_data, fingerprint)
            .await
    }
    ...
    async fn validate_certificate_simple(&self, certificate_der: &[u8]) -> Result<(), RustMqError> {
        ...
        self.validate_certificate_chain_internal(&certificates)
            .await?;
    ...
    pub fn get_statistics(&self) -> AuthStatistics {
        AuthStatistics {
            authentication_success_count: self
                .success_count
                .load(std::sync::atomic::Ordering::Relaxed),
            authentication_failure_count: self
                .failure_count
                .load(std::sync::atomic::Ordering::Relaxed),
        }
    }
