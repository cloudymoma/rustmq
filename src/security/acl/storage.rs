...
use super::{AclRule, AclOperation, Effect};
use crate::error::{Result, RustMqError};
use crate::storage::traits::{ObjectStorage, Cache};
...
pub trait AclStorage: Send + Sync {
    ...
    async fn create_rule(&self, rule: AclRule, created_by: &str, description: &str) -> Result<String>;
    
    /// Update an existing ACL rule
    async fn update_rule(&self, rule_id: &str, rule: AclRule, updated_by: &str, description: &str) -> Result<()>;
...
impl ObjectStorageAclStorage {
    ...
    fn serialize_rule(&self, rule: &VersionedAclRule) -> Result<Bytes> {
        let json = serde_json::to_string(rule)
            .map_err(|e| RustMqError::Config(e.to_string()))?;
...
impl AclStorage for ObjectStorageAclStorage {
    async fn create_rule(&self, mut rule: AclRule, created_by: &str, description: &str) -> Result<String> {
        ...
        if self.rule_index.read().await.contains_key(&rule.id) {
            return Err(RustMqError::AclEvaluation(format!("Rule with ID {} already exists", rule.id)));
        }
    ...
    async fn update_rule(&self, rule_id: &str, mut rule: AclRule, updated_by: &str, description: &str) -> Result<()> {
        // Ensure rule ID matches
        rule.id = rule_id.to_string();
        
        // Check if rule exists
        let existing_rule = self.get_rule(rule_id).await?
    ...
    async fn delete_rule(&self, rule_id: &str, deleted_by: &str, description: &str) -> Result<()> {
        // Get existing rule
        let mut existing_rule = self.get_rule(rule_id).await?
            .ok_or_else(|| RustMqError::AclEvaluation(format!("Rule {} not found", rule_id)))?;
    ...
    async fn list_rules(&self, filter: AclRuleFilter) -> Result<Vec<VersionedAclRule>> {
        ...
        for rule in all_rules {
            ...
            if let Some(ref operations) = filter.operations {
                if !operations.iter().any(|op| rule.rule.operations.contains(op)) {
                    continue;
                }
            }
    ...
    async fn get_rules_for_principal(&self, principal: &str) -> Result<Vec<VersionedAclRule>> {
        ...
        for (pattern, rule_ids) in principal_index.iter() {
            if pattern.contains('*') {
                ...
                if pattern == "*" || 
                   (pattern.ends_with('*') && principal.starts_with(&pattern[..pattern.len()-1])) ||
    ...
    async fn create_snapshot(&self) -> Result<AclSnapshot> {
        ...
        // Store snapshot
        let snapshot_key = format!("{}snapshots/{}.json", self.config.key_prefix, snapshot.snapshot_id);
        let data = serde_json::to_string(&snapshot)
            .map_err(|e| RustMqError::Config(e.to_string()))?;
        self.object_storage.put(&snapshot_key, Bytes::from(data)).await?;
    ...
    async fn restore_snapshot(&self, snapshot: AclSnapshot) -> Result<()> {
        ...
        for rule in snapshot.rules {
            if !rule.deleted {
                ...
                // Update indices
                self.rule_index.write().await.insert(rule.rule.id.clone(), object_key);
                self.principal_index.write().await
                    .entry(rule.rule.principal.clone())
                    .or_insert_with(HashSet::new)
                    .insert(rule.rule.id);
            }
...
mod tests {
    ...
    async fn create_test_storage() -> (ObjectStorageAclStorage, TempDir) {
        ...
        let storage = ObjectStorageAclStorage::new(object_storage, cache, config).await.unwrap();
        (storage, temp_dir)
    }
    ...
    async fn test_create_and_get_rule() {
        ...
        let rule_id = storage.create_rule(rule.clone(), "admin", "Initial rule").await.unwrap();
    ...
    async fn test_update_rule() {
        ...
        let rule_id = storage.create_rule(rule, "admin", "Initial rule").await.unwrap();
        ...
        storage.update_rule(&rule_id, updated_rule, "admin", "Updated rule").await.unwrap();
    ...
    async fn test_delete_rule() {
        ...
        storage.delete_rule(&rule_id, "admin", "Rule no longer needed").await.unwrap();
    ...
    async fn test_get_rules_for_principal() {
        ...
        storage.create_rule(rule, "admin", "Test rule").await.unwrap();
...
impl InMemoryAclStorage {
    ...
    pub async fn create_rule(&self, mut rule: AclRule) -> Result<()> {
        ...
        index.entry(rule.principal.clone())
            .or_insert_with(HashSet::new)
            .insert(rule.id.clone());
