...
use super::{
    AclEntry, AclOperation, AclRule, Effect, PolicyDecision,
    raft::RaftAclManager,
    storage::{AclPermission, AclRuleFilter, AclStorage, VersionedAclRule},
};
...
impl AclManager {
    ...
    pub async fn new(
        ...
        // Cache cleanup task
        {
            ...
            let task = tokio::spawn(async move {
                ...
                loop {
                    interval.tick().await;
                    Self::cleanup_expired_cache_entries(&l1_cache, &l2_cache_shards, cache_ttl)
                        .await;
                }
            });
    ...
    pub async fn evaluate_access(
        ...
        if let Some(cached_result) = self.check_l1_cache(&cache_key).await {
            let mut stats = self.cache_stats.write().await;
            stats.l1_hits += 1;
            return Ok(self.evaluate_cached_entry(
                &cached_result,
                principal,
                resource,
                &operation.into(),
        ...
        if let Some(cached_result) = self.check_l2_cache(&cache_key).await {
            ...
            // Promote to L1 cache
            self.update_l1_cache(cache_key.clone(), cached_result.clone())
                .await;
        ...
        let decision =
            self.evaluate_rules(&rules, principal, resource, &operation.into(), &context);

        // Cache the result
        if !rules.is_empty() {
            let cached_entry = self.create_cached_entry(rules[0].clone());
            self.update_l2_cache(cache_key.clone(), cached_entry.clone())
                .await;
    ...
    pub async fn sync_acls_to_broker(&self, broker_id: &str) -> Result<()> {
        ...
        self.network_handler
            .send_acl_sync(broker_id, sync_payload)
            .await
            .map_err(|e| {
                RustMqError::Network(format!(
                    "Failed to sync ACLs to broker {}: {}",
                    broker_id, e
                ))
            })?;
    ...
    pub async fn warm_cache(&self, principals: Vec<String>) -> Result<()> {
        for principal in principals {
            ...
            for rule in rules {
                if !rule.deleted {
                    let cache_key =
                        format!("{}:{}:*", rule.rule.principal, rule.rule.resource.pattern);
                    let cached_entry = self.create_cached_entry(rule);

                    // Add to both L1 and L2 caches
                    self.update_l1_cache(cache_key.clone(), cached_entry.clone())
                        .await;
    ...
    pub async fn store_acl_rules(&self, principal: &str, rules: Vec<AclRule>) -> Result<()> {
        for rule in rules {
            self.create_acl_rule(rule, "test", &format!("Test rule for {}", principal))
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn delete_acl_rules(&self, principal: &str) -> Result<()> {
        ...
        for rule in versioned_rules {
            self.delete_acl_rule(
                &rule.rule.id,
                "test",
                &format!("Test deletion for {}", principal),
            )
            .await?;
        }
        Ok(())
    ...
    pub async fn bulk_store_acl_rules(
        &self,
        rules_map: std::collections::HashMap<String, Vec<AclRule>>,
    ) -> Result<()> {
    ...
    pub async fn check_permission(
        ...
        self.evaluate_access(principal, resource, permission, None)
            .await
    }
    ...
    pub async fn create_rule(&self, rule: AclRule) -> Result<AclRule> {
        ...
        let rule_id = self
            .raft_manager
            .create_acl_rule(rule.clone(), "admin", "Created via Admin API")
            .await?;
    ...
    pub async fn update_rule(&self, id: String, rule: AclRule) -> Result<AclRule> {
        ...
        // Update the rule
        self.raft_manager
            .update_acl_rule(&id, rule.clone(), "admin", "Updated via Admin API")
            .await?;
    ...
    pub async fn delete_rule(&self, id: String) -> Result<()> {
        ...
        // Delete the rule
        self.raft_manager
            .delete_acl_rule(&id, "admin", "Deleted via Admin API")
            .await?;
    ...
    async fn update_l1_cache(&self, key: String, entry: CachedAclEntry) {
        ...
        if cache.len() >= (self.config.cache_size_mb * 1024 * 1024 / 1000) {
            // Rough estimation
    ...
    fn create_cached_entry(&self, versioned_rule: VersionedAclRule) -> CachedAclEntry {
        ...
        let entry = AclEntry::from_rule(rule_clone.clone()).unwrap_or_else(|_| {
            // Fallback to a basic entry if compilation fails
            AclEntry {
                rule: rule_clone.clone(),
                principal_pattern: super::rules::CompiledPattern::Exact(
                    rule_clone.principal.clone(),
                ),
                resource_pattern: super::rules::CompiledPattern::Exact(
                    rule_clone.resource.pattern.clone(),
                ),
            }
        });
    ...
    fn evaluate_rules(
        ...
        if matching_entries
            .iter()
            .any(|entry| entry.rule.effect == Effect::Allow)
        {
...
impl AclSecurityValidator {
    fn new() -> Self {
        Self {
            principal_patterns: vec![Regex::new(r"^[a-zA-Z0-9\-\._\*]+$").unwrap()],
            resource_patterns: vec![Regex::new(r"^[a-zA-Z0-9\-\._\*\/]+$").unwrap()],
    ...
    fn validate_rule(&self, rule: &AclRule) -> Result<()> {
        ...
        if !self
            .principal_patterns
            .iter()
            .any(|p| p.is_match(&rule.principal))
        {
            return Err(RustMqError::SecurityConfig(
                "Invalid principal pattern".to_string(),
            ));
        }
        ...
        if !self
            .resource_patterns
            .iter()
            .any(|p| p.is_match(&rule.resource.pattern))
        {
            return Err(RustMqError::SecurityConfig(
                "Invalid resource pattern".to_string(),
            ));
        }
        ...
        if rule.operations.is_empty() {
            return Err(RustMqError::SecurityConfig(
                "Rule must have at least one operation".to_string(),
            ));
        }
        ...
        if rule.conditions.len() > self.max_conditions {
            return Err(RustMqError::SecurityConfig(
                "Too many conditions in rule".to_string(),
            ));
        }
        ...
        for (key, value) in &rule.conditions {
            if key.len() > 100 || value.len() > 1000 {
                return Err(RustMqError::SecurityConfig(
                    "Condition key or value too long".to_string(),
                ));
            }
        }
...
impl AclManagerTrait for AclManager {
    async fn create_acl_rule(
        ...
        let result = self
            .raft_manager
            .create_acl_rule(rule.clone(), created_by, description)
            .await?;
    ...
    async fn update_acl_rule(
        ...
        self.raft_manager
            .update_acl_rule(rule_id, rule.clone(), updated_by, description)
            .await?;
    ...
    async fn delete_acl_rule(
        ...
        let rule = self
            .raft_manager
            .get_acl_rule(rule_id)
            .await?
            .ok_or_else(|| RustMqError::AclEvaluation(format!("Rule {} not found", rule_id)))?;

        self.raft_manager
            .delete_acl_rule(rule_id, deleted_by, description)
            .await?;
...
mod tests {
    ...
    async fn test_acl_manager_basic_evaluation() {
        ...
        use crate::security::acl::rules::{AclOperation, AclRule};
    ...
    async fn test_acl_manager_deny_precedence() {
        ...
        let rules = storage
            .get_rules_for_principal("test-principal")
            .await
            .unwrap();
