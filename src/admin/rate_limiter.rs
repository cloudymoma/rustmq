use crate::Result;
use crate::config::{EndpointCategoryLimits, RateLimitConfig};
use dashmap::DashMap;
use governor::{
    Quota, RateLimiter,
    clock::DefaultClock,
...
impl RateLimiterManager {
    ...
    pub fn new(config: RateLimitConfig) -> Result<Self> {
        ...
        let global_limiter = if config.enabled && config.global.requests_per_second > 0 {
            let quota = Quota::per_second(
                NonZeroU32::new(config.global.requests_per_second).ok_or_else(|| {
            ...
            .allow_burst(
                NonZeroU32::new(config.global.burst_capacity).ok_or_else(|| {
                    crate::error::RustMqError::InvalidConfig(
        ...
        info!(
            "Rate limiter manager initialized with {} category limiters",
    ...
    fn create_category_limiter(
        ...
        let quota = Quota::per_second(
            NonZeroU32::new(category_config.requests_per_second).ok_or_else(|| {
                crate::error::RustMqError::InvalidConfig(
        ...
        .allow_burst(
            NonZeroU32::new(category_config.burst_capacity).ok_or_else(|| {
                crate::error::RustMqError::InvalidConfig(
    ...
    pub async fn check_rate_limit(
        ...
        if let Some(global_limiter) = &self.global_limiter {
            if let Err(_) = global_limiter.check() {
                let retry_after =
                    self.calculate_retry_after(&self.config.global.requests_per_second);
        ...
        if self.config.per_ip.enabled {
            if let Err(_) = self.check_ip_rate_limit(info.client_ip).await {
                let retry_after =
                    self.calculate_retry_after(&self.config.per_ip.requests_per_second);
        ...
        if let Err(e) = self
            .check_category_rate_limit(&info.category, info.client_ip)
            .await
        {
            warn!(
                "Category rate limit exceeded for IP {} on {:?}",
                info.client_ip, info.category
            );
            return Err(e);
        }
        ...
        debug!(
            "Rate limit check passed for IP {} on {} {}",
            info.client_ip, info.method, info.endpoint
        );
    ...
    async fn check_ip_rate_limit(&self, client_ip: IpAddr) -> std::result::Result<(), ()> {
        ...
        let ip_limiter = self.ip_limiters.entry(client_ip).or_insert_with(|| {
            let quota =
                Quota::per_second(NonZeroU32::new(self.config.per_ip.requests_per_second).unwrap())
    ...
    async fn maybe_trigger_cleanup(&self) {
        ...
        if !cleanup_state.cleanup_running
            && cleanup_state.last_cleanup.elapsed().as_secs()
                >= self.config.cleanup.cleanup_interval_secs
        {
    ...
    async fn perform_cleanup(&self) {
        ...
        debug!(
            "Rate limiter cleanup completed. Active IP limiters: {}",
...
pub fn rate_limit_filter(manager: Arc<RateLimiterManager>) -> warp::filters::BoxedFilter<()> {
    extract_client_ip()
        .and(warp::method())
        .and(warp::path::full())
        ...
        .and_then(
            |client_ip: IpAddr,
             ...
             manager: Arc<RateLimiterManager>| async move {
...
pub async fn handle_rate_limit_rejection(
    ...
    if let Some(rate_limit_rejection) = rejection.find::<RateLimitRejection>() {
        ...
        let reply =
            warp::reply::with_header(reply, "X-RateLimit-Limit", rate_limit_rejection.0.limit);
        let reply = warp::reply::with_header(
            reply,
            "X-RateLimit-Remaining",
            rate_limit_rejection.0.remaining,
        );
        let reply = warp::reply::with_header(
            reply,
            "X-RateLimit-Reset",
            rate_limit_rejection.0.reset_time,
        );
        let reply =
            warp::reply::with_header(reply, "Retry-After", rate_limit_rejection.0.retry_after);
...
mod tests {
    use super::*;
    use crate::config::{
        EndpointCategoryLimits, EndpointRateLimits, GlobalRateLimits, PerIpRateLimits,
        RateLimitCleanupConfig, RateLimitConfig,
    };
