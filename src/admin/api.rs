...
impl BrokerHealthTracker {
    ...
    pub async fn check_broker_health(&self, broker: &BrokerInfo) -> bool {
        ...
        let health = cache
            .entry(broker.id.clone())
            .or_insert_with(|| BrokerHealth {
                online: false,
        ...
        if is_healthy {
            ...
            debug!(
                "Health check failed for broker {} (consecutive failures: {})",
    ...
    async fn perform_health_check(&self, broker: &BrokerInfo) -> bool {
                        ...
                        .await
                    {
                        Ok(channel) => {
                            ...
                            match client.health_check(request).await {
                                ...
                                Err(e) => {
                                    debug!(
                                        "Health check RPC failed for broker {}: {}",
                                        broker.id, e
                                    );
                                    false
                                }
        ...
        match check_result {
            ...
            Err(_) => {
                debug!(
                    "Health check timeout for broker {} at {}",
                    broker.id, endpoint
                );
                false
            }
...
impl AdminApi {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        let all_routes = api_routes.or(static_files).boxed();
        ...
        let rate_limit_status = if self.rate_limiter.is_some() {
            "enabled"
        } else {
            "disabled"
        };
        let security_status = if self.security_api.is_some() {
            "enabled"
        } else {
            "disabled"
        };
        info!(
            "Starting Admin API server on port {} (rate limiting: {}, security: {})",
...
async fn handle_cluster_status(
    ...
    match controller.get_cluster_metadata().await {
        Ok(metadata) => {
            ...
            let broker_statuses: Vec<BrokerStatus> = metadata
                .brokers
                .into_iter()
                .map(|b| {
        ...
        Ok(create_response) => {
            if create_response.success {
                ...
                warn!(
                    ...
                    request.name, create_response.error_message
                );
        ...
        Ok(delete_response) => {
            if delete_response.success {
                ...
                warn!(
                    ...
                    topic_name, delete_response.error_message
                );
                let response = ApiResponse::<String> {
                    success: false,
                    data: None,
...
async fn handle_list_brokers(
    ...
    match controller.get_cluster_metadata().await {
        Ok(metadata) => {
            ...
            let brokers: Vec<BrokerStatus> = metadata
                .brokers
                .into_iter()
                .map(|b| {
                    let is_online = broker_health.get(&b.id).copied().unwrap_or(false);
...
async fn handle_rejection_unified(
    err: Rejection,
) -> std::result::Result<warp::reply::Response, Infallible> {
...
mod tests {
    ...
    async fn test_admin_api_with_rate_limiting() {
        ...
        let admin_api =
            AdminApi::new_with_rate_limiting(controller, 8080, rate_limit_config).unwrap();
    ...
    async fn test_admin_api_with_security() {
        ...
        use crate::security::{
            AclConfig, AuditConfig, CertificateManagementConfig, SecurityConfig, SecurityManager,
        };
    ...
    async fn test_rate_limiter_manager_integration() {
        use crate::admin::rate_limiter::{EndpointCategory, RateLimitInfo, RateLimiterManager};
        use crate::config::{
            EndpointCategoryLimits, EndpointRateLimits, GlobalRateLimits, PerIpRateLimits,
            RateLimitCleanupConfig, RateLimitConfig,
        };
