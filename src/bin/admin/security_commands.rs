...
use super::api_client::{
    AclEvaluationRequest, AdminApiClient, BulkAclEvaluationRequest, CertificateValidationRequest,
    CreateAclRuleRequest, GenerateCaRequest, IssueCertificateRequest, MaintenanceRequest,
    RevokeCertificateRequest, SecurityAuditLogRequest, UpdateAclRuleRequest,
};
use super::formatters::{
    ProgressIndicator, confirm_operation, print_error, print_info, print_output, print_success,
    print_warning,
};
use crate::Cli;
...
pub async fn execute_ca_command(
    ...
    match cmd {
        CaCommands::Init(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/ca/generate", &request)
                .await?;
        ...
        CaCommands::List(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = if query.is_empty() {
                ...
                api_client
                    .get_with_query("/api/v1/security/ca/list", &query)
                    .await?
            };
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to list CAs".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CaCommands::Info(args) => {
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get CA info".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
    ...
    match cmd {
        CertCommands::Issue(args) => {
            ...
            let request = IssueCertificateRequest {
                ...
                subject_alt_names: if args.san.is_empty() {
                    None
                } else {
                    Some(args.san.clone())
                },
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/certificates/issue", &request)
                .await?;
        ...
        CertCommands::List(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = if query.is_empty() {
                ...
                api_client
                    .get_with_query("/api/v1/security/certificates", &query)
                    .await?
            };
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to list certificates".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Info(args) => {
            ...
            if response.success {
                if let Some(data) = &response.data {
                    ...
                    print_error(
                        &format!("Certificate '{}' not found", args.cert_id),
                        cli.no_color,
                    );
                    std::process::exit(1);
                }
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get certificate info".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Renew(args) => {
            ...
            if response.success {
                print_success(
                    &format!("Certificate '{}' renewed successfully", args.cert_id),
                    cli.no_color,
                );
                if let Some(data) = &response.data {
                    print_output(data, cli.format, cli.no_color)?;
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to renew certificate".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Rotate(args) => {
            ...
            if response.success {
                print_success(
                    &format!("Certificate '{}' rotated successfully", args.cert_id),
                    cli.no_color,
                );
                if let Some(data) = &response.data {
                    print_output(data, cli.format, cli.no_color)?;
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to rotate certificate".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Revoke(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> =
                api_client.post(&path, &request).await?;

            if response.success {
                print_success(
                    &format!("Certificate '{}' revoked successfully", args.cert_id),
                    cli.no_color,
                );
                if let Some(data) = &response.data {
                    print_output(data, cli.format, cli.no_color)?;
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to revoke certificate".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Status(args) => {
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get certificate status".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Expiring(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .get_with_query("/api/v1/security/certificates/expiring", &query)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get expiring certificates".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        CertCommands::Validate(args) => {
            ...
            let cert_pem = std::fs::read_to_string(&args.cert_file).map_err(|e| {
                rustmq::error::RustMqError::Config(format!(
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/certificates/validate", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to validate certificate".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
            ...
            if response.success {
                if let Some(data) = &response.data {
                    if let Some(output_path) = &args.output {
                        ...
                        print_success(
                            &format!("Certificate exported to: {:?}", output_path),
                            cli.no_color,
                        );
                    } else {
                        // Print to stdout
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to export certificate".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
    ...
    match cmd {
        AclCommands::Create(args) => {
            ...
            for condition in &args.conditions {
                if let Some((key, value)) = condition.split_once('=') {
                    ...
                    print_error(
                        &format!(
                            "Invalid condition format: '{}'. Use key=value format",
                            condition
                        ),
                        cli.no_color,
                    );
            ...
            for permission in permissions {
                ...
                let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                    .post("/api/v1/security/acl/rules", &request)
                    .await?;

                if !response.success {
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Failed to create ACL rule".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
            }
        ...
        AclCommands::List(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = if query.is_empty() {
                ...
                api_client
                    .get_with_query("/api/v1/security/acl/rules", &query)
                    .await?
            };
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to list ACL rules".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Info(args) => {
            ...
            if response.success {
                if let Some(data) = &response.data {
                    ...
                    print_error(
                        &format!("ACL rule '{}' not found", args.rule_id),
                        cli.no_color,
                    );
                    std::process::exit(1);
                }
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get ACL rule info".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Update(args) => {
            ...
            let path = format!("/api/v1/security/acl/rules/{}", args.rule_id);
            let response: super::api_client::ApiResponse<serde_json::Value> =
                api_client.put(&path, &request).await?;

            if response.success {
                print_success(
                    &format!("ACL rule '{}' updated successfully", args.rule_id),
                    cli.no_color,
                );
                if let Some(data) = &response.data {
                    print_output(data, cli.format, cli.no_color)?;
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to update ACL rule".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Delete(args) => {
            ...
            if response.success {
                print_success(
                    &format!("ACL rule '{}' deleted successfully", args.rule_id),
                    cli.no_color,
                );
            } else {
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to delete ACL rule".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Test(args) => {
            debug!(
                "Testing ACL evaluation for: {} on {}",
                args.principal, args.resource
            );
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/acl/evaluate", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to evaluate ACL".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Permissions(args) => {
            ...
            let path = format!(
                "/api/v1/security/acl/principals/{}/permissions",
                args.principal
            );
            let response = api_client.get::<serde_json::Value>(&path).await?;
            ...
            if response.success {
                if let Some(data) = &response.data {
                    ...
                    print_info(
                        &format!("No permissions found for principal '{}'", args.principal),
                        cli.no_color,
                    );
                }
            } else {
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get principal permissions".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Rules(args) => {
            ...
            if response.success {
                if let Some(data) = &response.data {
                    ...
                    print_info(
                        &format!("No rules found for resource '{}'", args.resource),
                        cli.no_color,
                    );
                }
            } else {
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get resource rules".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::BulkTest(args) => {
            debug!(
                "Running bulk ACL evaluation from file: {:?}",
                args.input_file
            );

            let input_content = std::fs::read_to_string(&args.input_file).map_err(|e| {
                rustmq::error::RustMqError::Config(format!("Failed to read input file: {}", e))
            })?;

            let request: BulkAclEvaluationRequest = serde_json::from_str(&input_content)?;

            let progress = ProgressIndicator::new(
                &format!("Evaluating {} ACL requests", request.evaluations.len()),
                cli.no_color,
            );
            progress.start();

            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/acl/bulk-evaluate", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to perform bulk ACL evaluation".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::Sync(args) => {
            if args.force || confirm_operation("sync ACL rules to all brokers", "cluster") {
                ...
                if response.success {
                    ...
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Failed to sync ACL rules".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
            } else {
        ...
        AclCommands::Version => {
            ...
            let response = api_client
                .get::<serde_json::Value>("/api/v1/security/acl/version")
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get ACL version".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::CacheInvalidate(args) => {
            ...
            let response = api_client
                .post::<serde_json::Value, serde_json::Value>(
                    "/api/v1/security/acl/cache/invalidate",
                    &serde_json::json!({}),
                )
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to invalidate ACL caches".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AclCommands::CacheWarm(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/acl/cache/warm", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to warm ACL caches".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
...
pub async fn execute_audit_command(
    ...
    match cmd {
        AuditCommands::Logs(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .get_with_query("/api/v1/security/audit/logs", &query)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get audit logs".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AuditCommands::Events(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .get_with_query("/api/v1/security/audit/events", &query)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get security events".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AuditCommands::Certificates(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .get_with_query("/api/v1/security/audit/certificates", &query)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get certificate audit trail".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        AuditCommands::Acl(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .get_with_query("/api/v1/security/audit/acl", &query)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get ACL audit trail".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
...
pub async fn execute_security_command(
    ...
    match cmd {
        SecurityCommands::Status => {
            ...
            let response = api_client
                .get::<serde_json::Value>("/api/v1/security/status")
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get security status".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        SecurityCommands::Metrics => {
            ...
            let response = api_client
                .get::<serde_json::Value>("/api/v1/security/metrics")
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get security metrics".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        SecurityCommands::Health => {
            ...
            let response = api_client
                .get::<serde_json::Value>("/api/v1/security/health")
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get security health".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        SecurityCommands::Config => {
            ...
            let response = api_client
                .get::<serde_json::Value>("/api/v1/security/configuration")
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get security configuration".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        SecurityCommands::Cleanup(args) => {
            ...
            for operation in operations {
                ...
                let progress =
                    ProgressIndicator::new(&format!("Running {}", operation), cli.no_color);
                progress.start();

                let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                    .post("/api/v1/security/maintenance/cleanup", &request)
                    .await?;
                ...
                if response.success {
                    ...
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Failed to perform cleanup".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
            }
        ...
        SecurityCommands::Backup(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/maintenance/backup", &request)
                .await?;

            if response.success {
                progress.finish(true);
                print_success(
                    &format!("Security configuration backed up to: {:?}", args.output),
                    cli.no_color,
                );
                if let Some(data) = &response.data {
                    print_output(data, cli.format, cli.no_color)?;
        ...
        SecurityCommands::Restore(args) => {
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/maintenance/restore", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to restore configuration".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
    ...
    match cmd {
        WebpkiCommands::Validate(args) => {
            ...
            let ca_pem = if let Some(ca_file) = &args.ca_file {
                Some(std::fs::read_to_string(ca_file).map_err(|e| {
                    rustmq::error::RustMqError::Config(format!("Failed to read CA file: {}", e))
                })?)
            } else {
                None
            };
            ...
            let progress =
                ProgressIndicator::new("Validating certificate with WebPKI", cli.no_color);
            progress.start();

            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/webpki/validate", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "WebPKI validation failed".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        WebpkiCommands::ValidateChain(args) => {
            info!(
                "Validating certificate chain using WebPKI: {:?}",
                args.chain_file
            );

            // Read certificate chain file
            let chain_pem = std::fs::read_to_string(&args.chain_file).map_err(|e| {
                rustmq::error::RustMqError::Config(format!(
            ...
            let trust_anchors_pem = if let Some(trust_file) = &args.trust_anchors_file {
                Some(std::fs::read_to_string(trust_file).map_err(|e| {
                    rustmq::error::RustMqError::Config(format!(
                        "Failed to read trust anchors file: {}",
                        e
                    ))
                })?)
            } else {
            ...
            if let Some(verification_time) = verification_time {
                request["verification_time"] =
                    serde_json::Value::String(verification_time.to_rfc3339());
            }

            let progress =
                ProgressIndicator::new("Validating certificate chain with WebPKI", cli.no_color);
            progress.start();

            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/webpki/validate-chain", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "WebPKI chain validation failed".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        WebpkiCommands::TrustAnchors(args) => {
            if args.list {
                ...
                let response = api_client
                    .get::<serde_json::Value>("/api/v1/security/webpki/trust-anchors")
                    .await?;
                ...
                if response.success {
                    ...
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Failed to list trust anchors".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
                ...
                let trust_anchor_pem = std::fs::read_to_string(add_file).map_err(|e| {
                    rustmq::error::RustMqError::Config(format!(
                ...
                let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                    .post("/api/v1/security/webpki/trust-anchors", &request)
                    .await?;
                ...
                if response.success {
                    ...
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Failed to add trust anchor".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
                ...
                if response.success {
                    print_success(
                        &format!("Trust anchor '{}' removed successfully", thumbprint),
                        cli.no_color,
                    );
                } else {
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Failed to remove trust anchor".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
                ...
                let response = api_client
                    .post::<serde_json::Value, serde_json::Value>(
                        "/api/v1/security/webpki/trust-anchors/validate",
                        &serde_json::json!({}),
                    )
                    .await?;
                ...
                if response.success {
                    ...
                    let error_msg = response
                        .error
                        .unwrap_or_else(|| "Trust anchor validation failed".to_string());
                    print_error(&error_msg, cli.no_color);
                    std::process::exit(1);
                }
            } else {
                print_error(
                    "No trust anchor operation specified. Use --list, --add-file, --remove, or --validate",
                    cli.no_color,
                );
                std::process::exit(1);
            }
        ...
        WebpkiCommands::Benchmark(args) => {
            info!(
                "Running WebPKI validation benchmark with {} iterations",
                args.iterations
            );
            ...
            let progress = ProgressIndicator::new(
                &format!("Running {} validation iterations", args.iterations),
                cli.no_color,
            );
            progress.start();

            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/webpki/benchmark", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "WebPKI benchmark failed".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        WebpkiCommands::Status => {
            ...
            let response = api_client
                .get::<serde_json::Value>("/api/v1/security/webpki/status")
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Failed to get WebPKI status".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
        WebpkiCommands::Compare(args) => {
            info!(
                "Comparing validation methods for certificate: {:?}",
                args.cert_file
            );
            ...
            let response: super::api_client::ApiResponse<serde_json::Value> = api_client
                .post("/api/v1/security/webpki/compare", &request)
                .await?;
            ...
            if response.success {
                ...
                let error_msg = response
                    .error
                    .unwrap_or_else(|| "Validation comparison failed".to_string());
                print_error(&error_msg, cli.no_color);
                std::process::exit(1);
            }
        }
