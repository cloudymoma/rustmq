...
use admin::{
    AclCommands, AdminApiClient, AuditCommands, CaCommands, CertCommands, OutputFormat,
    SecurityCommands, execute_acl_command, execute_audit_command, execute_ca_command,
    execute_cert_command, execute_security_command,
};
...
async fn main() -> Result<()> {
    ...
    match &cli.command {
                        ...
                        Ok(response) => {
                            if response.success {
                                ...
                                if let Some(error) = response.error_message {
                                    ...
                                    error!(
                                        "❌ Failed to create topic '{}': Unknown error",
                                        topic_name
                                    );
                                    std::process::exit(1);
                                }
        ...
        Commands::Topic(TopicCommands::List) => {
            ...
            match create_admin_controller().await {
                Ok(controller) => match controller.get_cluster_metadata().await {
                    Ok(metadata) => {
                        if metadata.topics.is_empty() {
                            ...
                            println!(
                                "{:<30} {:<12} {:<18} {:<20}",
                                "NAME", "PARTITIONS", "REPLICATION", "CREATED"
                            );
                            println!("{}", "=".repeat(80));
                            ...
                            for topic in metadata.topics {
                                ...
                                println!(
                                    "{:<30} {:<12} {:<18} {:<20}",
                                    topic.name, topic.partitions, topic.replication_factor, created
                                );
                            }
                        }
                    }
            ...
            match create_admin_controller().await {
                Ok(controller) => {
                    match controller.get_cluster_metadata().await {
                        Ok(metadata) => {
                            if let Some(topic) =
                                metadata.topics.iter().find(|t| t.name == *topic_name)
                            {
                                ...
                                println!(
                                    "Created:             {}",
                                    topic.created_at.format("%Y-%m-%d %H:%M:%S UTC")
                                );
                        ...
                        Ok(response) => {
                            if response.success {
                                ...
                                if let Some(error) = response.error_message {
                                    ...
                                    error!(
                                        "❌ Failed to delete topic '{}': Unknown error",
                                        topic_name
                                    );
                                    std::process::exit(1);
                                }
                ...
                Ok(controller) => {
                    match controller.get_cluster_metadata().await {
                        Ok(metadata) => {
                            ...
                            println!(
                                "Leader:              {}",
                                metadata.leader.as_deref().unwrap_or("none")
                            );
                            println!("Term:                {}", metadata.term);
                            println!(
                                "Raft Status:         {}",
                                if raft_info.is_leader {
                                    "Leader"
                                } else {
                                    "Follower"
                                }
                            ...
                            if metadata.brokers.is_empty() {
                                ...
                                for broker in &metadata.brokers {
                                    println!(
                                        "  ✅ {} ({}:{}) - Rack: {}",
                                        broker.id, broker.host, broker.port_quic, broker.rack_id
                                    );
                                }
                            }
                            ...
                            if metadata.topics.is_empty() {
                                ...
                                for topic in &metadata.topics {
                                    let topic_partitions: Vec<_> = metadata
                                        .partition_assignments
                                        .iter()
                            ...
                            let all_topics_healthy = metadata.topics.iter().all(|topic| {
                                ...
                                topic_partitions
                                    .iter()
                                    .all(|(_, assignment)| !assignment.leader.is_empty())
                            });
                            ...
                            if has_leader && has_brokers && all_topics_healthy {
                                ...
                                if !all_topics_healthy {
                                    println!(
                                        "  Issue: Some topics have partitions without leaders"
                                    );
                                }
                            }
                        }
        ...
        // Security commands
        Commands::Ca(ca_cmd) => execute_ca_command(ca_cmd, &api_client, &cli).await?,
        Commands::Certs(cert_cmd) => execute_cert_command(cert_cmd, &api_client, &cli).await?,
        Commands::Acl(acl_cmd) => execute_acl_command(acl_cmd, &api_client, &cli).await?,
        Commands::Audit(audit_cmd) => execute_audit_command(audit_cmd, &api_client, &cli).await?,
...
mod tests {
    ...
    async fn test_topic_description_flow() {
        ...
        let topic = metadata
            .topics
            .iter()
            .find(|t| t.name == "admin-describe-topic")
            .unwrap();
    ...
    async fn test_topic_configuration_options() {
        ...
        let topic = metadata
            .topics
            .iter()
            .find(|t| t.name == "config-test-topic")
            .unwrap();
