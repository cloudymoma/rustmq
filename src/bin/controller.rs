...
use rustmq::controller::{
    CompactionConfig, HealthCheckResult, ManagerState, NodeId, PerformanceConfig, RaftManager,
    RaftManagerConfig, RustMqAppData, RustMqAppDataResponse, RustMqNetworkConfig, RustMqNode,
...
async fn main() -> Result<()> {
    ...
    raft_manager
        .wait_for_state(ManagerState::Running, Duration::from_secs(30))
        .await?;
    ...
    if config.controller.endpoints.len() > 1 {
        info!(
            "Setting up multi-node cluster with {} endpoints",
            config.controller.endpoints.len()
        );
            ...
            loop {
                ...
                match health {
                    HealthCheckResult::Healthy => {
                        info!(
                            "🟢 OpenRaft cluster healthy - State: {:?}, Leader: {}",
                            state, is_leader
                        );
                    }
                    HealthCheckResult::Degraded(reason) => {
                        warn!(
                            "🟡 OpenRaft cluster degraded - State: {:?}, Reason: {}",
                            state, reason
                        );
                    }
                    HealthCheckResult::Unhealthy(reason) => {
                        error!(
                            "🔴 OpenRaft cluster unhealthy - State: {:?}, Reason: {}",
                            state, reason
                        );
                    }
                ...
                if let Some(ref raft_metrics) = metrics.raft_metrics {
                    debug!(
                        "📊 Raft metrics - Term: {}, Last log: {:?}, State: {:?}",
                        raft_metrics.current_term, raft_metrics.last_log_index, raft_metrics.state
                    );
                }
            }
        })
