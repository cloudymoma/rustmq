...
use crate::controller::{
    TopicConfig as ServiceTopicConfig, WorkingAppData, WorkingNodeId, WorkingOpenRaftManager,
    WorkingRaftConfig, WorkingRaftIntegrationHelper,
};
...
async fn test_working_topic_operations() {
    ...
    // Test topic deletion
    let result = manager
        .delete_topic("working-test-topic".to_string())
        .await
        .unwrap();
    assert!(result.success);
...
async fn test_working_broker_management() {
    ...
    // Remove broker
    let result = manager
        .remove_broker("working-broker-1".to_string())
        .await
        .unwrap();
    assert!(result.success);
...
async fn test_working_integration_helper() {
    ...
    let result = helper
        .create_topic("helper-test-topic".to_string(), 3, 2, config)
        .await
        .unwrap();
    ...
    // Test topic deletion
    let result = helper
        .delete_topic("helper-test-topic".to_string())
        .await
        .unwrap();
    assert!(result.success);
...
async fn test_working_leadership_requirements() {
    ...
    let result = helper
        .create_topic("no-leader-topic".to_string(), 3, 2, config)
        .await
        .unwrap();
    ...
    let result = helper
        .create_topic("leader-topic".to_string(), 3, 2, config)
        .await
        .unwrap();
...
async fn test_working_multiple_operations() {
    ...
    for i in 1..=5 {
        ...
        let result = manager
            .create_topic(format!("topic-{}", i), 3, 1, topic_config)
            .await
            .unwrap();
...
async fn test_working_concurrent_operations() {
    ...
    for i in 1..=10 {
        assert!(
            metadata
                .brokers
                .contains_key(&format!("concurrent-broker-{}", i))
        );
    }
...
async fn test_working_error_cases() {
    ...
    let result = manager
        .create_topic("error-test-topic".to_string(), 3, 1, topic_config.clone())
        .await
        .unwrap();
    assert!(result.success);
    ...
    let result = manager
        .create_topic("error-test-topic".to_string(), 3, 1, topic_config)
        .await
        .unwrap();
    assert!(!result.success);
    assert!(result.error_message.is_some());
    ...
    let result = manager
        .delete_topic("non-existent-topic".to_string())
        .await
        .unwrap();
    assert!(!result.success);
    assert!(result.error_message.is_some());
    ...
    let result = manager
        .remove_broker("non-existent-broker".to_string())
        .await
        .unwrap();
    assert!(!result.success);
    assert!(result.error_message.is_some());
...
async fn test_working_state_persistence() {
    ...
    manager
        .create_topic("persistent-topic".to_string(), 3, 1, topic_config)
        .await
        .unwrap();
