...
impl Metrics {
    pub fn new() -> Arc<Self> {
        ...
        let messages_produced = Counter::new("messages_produced_total", "Total number of messages produced")
            .expect("Failed to create messages_produced counter");
        
        let messages_consumed = Counter::new("messages_consumed_total", "Total number of messages consumed")
            .expect("Failed to create messages_consumed counter");
        
        let produce_latency = Histogram::with_opts(
            prometheus::HistogramOpts::new("produce_latency_seconds", "Produce request latency")
        ).expect("Failed to create produce_latency histogram");
        
        let consume_latency = Histogram::with_opts(
            prometheus::HistogramOpts::new("consume_latency_seconds", "Consume request latency")
