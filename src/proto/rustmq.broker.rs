...
pub mod broker_replication_service_server {
    ...
    #[async_trait]
    pub trait BrokerReplicationService: std::marker::Send + std::marker::Sync + 'static {
        /// Replicate data from leader to follower
        /// CRITICAL: Must validate leader epoch to prevent stale leader attacks
    ...
    /// This service implements the critical leader-follower replication protocol
    #[derive(Debug)]
    pub struct BrokerReplicationServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> BrokerReplicationServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for BrokerReplicationServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "rustmq.broker.BrokerReplicationService";
    impl<T> tonic::server::NamedService for BrokerReplicationServiceServer<T> {
