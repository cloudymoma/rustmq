...
impl ReplicationManager {
    ...
    async fn send_to_follower(&self, broker_id: BrokerId, record: WalRecord) -> Result<()> {
        ...
        let leader_id = self
            .current_leader
            .as_ref()
            .ok_or(crate::error::RustMqError::NoLeader)?
            .clone();
        ...
        let response = timeout(
            self.ack_timeout,
            self.rpc_client.replicate_data(&broker_id, request),
        )
        .await
    ...
    async fn wait_for_acknowledgments(
        ...
        let followers: Vec<BrokerId> = self
            .replica_set
            .iter()
    ...
    async fn recalculate_high_watermark(&self) {
        ...
        if offsets.len() >= self.min_in_sync_replicas {
            ...
            if new_high_watermark > current_hwm {
                self.high_watermark
                    .store(new_high_watermark, Ordering::SeqCst);
                tracing::debug!(
    ...
    pub fn find_kth_smallest(offsets: &[u64], k: usize) -> u64 {
        debug_assert!(
            k > 0 && k <= offsets.len(),
            "k must be in range [1, offsets.len()]"
        );
            ...
            loop {
                ...
                if let Some(ref leader_id) = current_leader {
                    ...
                    for broker_id in followers {
                        match rpc_client
                            .send_heartbeat(&broker_id, heartbeat_request.clone())
                            .await
                        {
                            Ok(response) => {
                                if response.success {
...
impl ReplicationManagerTrait for ReplicationManager {
    async fn replicate_record(&self, record: &WalRecord) -> Result<ReplicationResult> {
        ...
        self.wait_for_acknowledgments(record.clone(), local_offset)
            .await
    }
...
impl ReplicationManager {
    ...
    pub fn get_last_heartbeat_time(&self, broker_id: &BrokerId) -> Option<std::time::Instant> {
        ...
        states.get(broker_id).map(|state| {
            ...
            let duration_since_heartbeat = (now_chrono - state.last_heartbeat)
                .to_std()
...
mod tests {
    ...
    async fn test_replication_manager() {
        ...
        assert!(
            states.len() >= 2,
            "Expected at least 2 follower states (1 from replication + broker-4), got {}",
            states.len()
        );
        assert!(
            states.len() <= 3,
        ...
        assert!(
            broker_4_state.is_some(),
            "broker-4 should be present in follower states"
        );
    }
    ...
    async fn test_high_watermark_advancement() {
        ...
        replication_manager
            .update_follower_state(follower_state_2)
            .await;
        replication_manager
            .update_follower_state(follower_state_3)
            .await;
    ...
    async fn test_isr_heartbeat_tracking() {
        ...
        replication_manager
            .update_follower_state(fresh_follower)
            .await;
        ...
        replication_manager
            .update_follower_state(stale_follower)
            .await;
        let isr = replication_manager.get_in_sync_replicas();
    ...
    async fn test_heartbeat_timeout_error_handling() {
        ...
        replication_manager
            .update_follower_state(invalid_follower)
            .await;
        ...
        assert_eq!(
            isr.len(),
            0,
            "Follower with invalid heartbeat timestamp should be excluded"
        );
        ...
        replication_manager
            .update_follower_state(valid_follower)
            .await;
        let isr = replication_manager.get_in_sync_replicas();
        assert_eq!(
            isr.len(),
            1,
            "Follower with valid heartbeat timestamp should be included"
        );
    }
    ...
    async fn test_optimized_high_watermark_calculation() {
        ...
        assert_eq!(
            hwm, 98,
            "High-watermark should be calculated correctly using optimized algorithm"
        );
        ...
        replication_manager
            .update_follower_state(lagging_follower)
            .await;
        replication_manager.recalculate_high_watermark().await;
    ...
    fn test_correctness_edge_cases() {
        ...
        for (offsets, k, expected) in test_cases {
            ...
            assert_eq!(
                optimized, expected,
                "Edge case failed: find_kth_smallest({:?}, {}) should return {}",
                offsets, k, expected
            );
            assert_eq!(
                optimized, original,
                "Optimized result must match original for offsets={:?}, k={}",
                offsets, k
            );
        }
    }
    ...
    fn test_correctness_replication_scenarios() {
        ...
        let scenarios = vec![
            ...
            (
                vec![u64::MAX - 100, u64::MAX - 50, u64::MAX],
                2,
                u64::MAX - 50,
            ), // 2nd smallest (sorted: [MAX-100, MAX-50, MAX])
        ...
        for (offsets, k, expected) in scenarios {
            ...
            assert_eq!(
                optimized, expected,
                "Replication scenario failed: offsets={:?}, k={}, expected={}",
                offsets, k, expected
            );
            assert_eq!(
                optimized, original,
                "Optimized must match original for replication scenario: offsets={:?}, k={}",
                offsets, k
            );
        }
    }
    ...
    fn test_correctness_duplicate_values() {
        ...
        for (offsets, k, expected) in test_cases {
            ...
            assert_eq!(
                optimized, expected,
                "Duplicate values test failed: offsets={:?}, k={}, expected={}",
                offsets, k, expected
            );
            assert_eq!(
                optimized, original,
                "Optimized must match original for duplicates: offsets={:?}, k={}",
                offsets, k
            );
        }
    }
    ...
    async fn test_high_watermark_calculation_correctness() {
        ...
        for (min_isr, leader_offset, follower_offsets, expected_hwm) in test_scenarios {
            ...
            replication_manager
                .log_end_offset
                .store(leader_offset, Ordering::SeqCst);
            ...
            for (i, &offset) in follower_offsets.iter().enumerate() {
                ...
                replication_manager
                    .update_follower_state(follower_state)
                    .await;
            }
            ...
            if all_offsets.len() >= min_isr {
                ...
                let optimized_result =
                    ReplicationManager::find_kth_smallest(&all_offsets, k_from_bottom);
    ...
    fn test_find_kth_smallest_correctness() {
        ...
        for (offsets, k, expected) in test_cases {
            ...
            assert_eq!(
                result, expected,
                "find_kth_smallest({:?}, {}) should return {}",
                offsets, k, expected
            );
            assert_eq!(
                result, naive_result,
...
mod benchmarks {
    ...
    fn test_memory_efficiency() {
        ...
        let test_cases = vec![(100, 3), (1000, 3), (10000, 3), (100000, 3)];
