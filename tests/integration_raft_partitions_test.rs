...
async fn test_raft_network_partition() {
    ...
    nodes.insert(1, RustMqNode { addr: "127.0.0.1".to_string(), rpc_port: 9101, data: "".to_string() });
    nodes.insert(2, RustMqNode { addr: "127.0.0.1".to_string(), rpc_port: 9102, data: "".to_string() });
    nodes.insert(3, RustMqNode { addr: "127.0.0.1".to_string(), rpc_port: 9103, data: "".to_string() });
    ...
    let mut leader_id = 0;
    for _ in 0..20 {
        if node1.is_leader().await { leader_id = 1; break; }
        if node2.is_leader().await { leader_id = 2; break; }
        if node3.is_leader().await { leader_id = 3; break; }
    ...
    let follower_to_block = if leader_id == 1 { 3 } else if leader_id == 2 { 3 } else { 1 };
    
    println!("Simulating partition: Blocking communication from Node {} to Node {}", leader_id, follower_to_block);
    
    leader.network().unwrap().block_node(follower_to_block).await;
    ...
    assert!(response.is_ok(), "Leader should still be able to commit with majority");
    ...
    let other_follower = if leader_id == 1 { 2 } else if leader_id == 2 { 1 } else { 2 };
    println!("Simulating partition: Blocking communication from Node {} to Node {}", leader_id, other_follower);
    ...
    let response = tokio::time::timeout(Duration::from_secs(5), leader.apply_command(app_data)).await;
    assert!(response.is_err() || response.unwrap().is_err(), "Leader should fail to commit when isolated");
