...
fn authorization_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("different_principals", |b| {
        let principals: Vec<Arc<str>> =
            (0..100).map(|i| Arc::from(format!("user_{}", i))).collect();
    ...
    group.bench_function("different_topics", |b| {
        let topics: Vec<String> = (0..50).map(|i| format!("topic_{}", i)).collect();
        let principal: Arc<str> = Arc::from("benchmark_user");
        ...
        b.to_async(&rt).iter(|| {
            ...
            async move {
                ...
                let result = auth_manager
                    .check_permission(&connection_cache, &principal, topic, Permission::Read)
                    .await;
                black_box(result)
            }
        });
    ...
    group.bench_function("different_permissions", |b| {
        ...
        b.to_async(&rt).iter(|| {
            ...
            async move {
                ...
                let result = auth_manager
                    .check_permission(&connection_cache, &principal, "benchmark_topic", permission)
                    .await;
                black_box(result)
            }
        });
...
fn invalidation_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("single_principal", |b| {
        let principals = [
            "user1", "user2", "user3", "user4", "user5", "admin1", "admin2", "service1",
            "service2", "broker1",
        ];
