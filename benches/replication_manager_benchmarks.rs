                ...
                group.bench_with_input(
                    ...
                    |b, &(n, k, scenario)| {
                        b.iter_batched(
                            ...
                            |offsets| {
                                benchmark_original_approach(black_box(&offsets), black_box(k))
                            },
                ...
                group.bench_with_input(
                    ...
                    |b, &(n, k, scenario)| {
                        b.iter_batched(
                            || generate_test_offsets(n, scenario),
                            |offsets| {
                                benchmark_optimized_approach(black_box(&offsets), black_box(k))
                            },
                            BatchSize::SmallInput,
                        );
                    },
...
fn bench_special_cases(c: &mut Criterion) {
    ...
    for &n in &[100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("optimized_k1", &id), &n, |b, &n| {
            b.iter_batched(
    ...
    for &n in &[100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("optimized_k2", &id), &n, |b, &n| {
            b.iter_batched(
                || generate_test_offsets(n, "worst_case"),
...
fn bench_correctness_verification(_c: &mut Criterion) {
    ...
    let test_cases = vec![(100, 2), (100, 3), (100, 5), (1000, 3), (10000, 3)];
    ...
    for (n, k) in test_cases {
        for scenario in &["sequential", "spread", "duplicates", "worst_case"] {
            ...
            assert_eq!(
                original, naive,
                "Original and naive should match for n={}, k={}, scenario={}",
                n, k, scenario
            );
            assert_eq!(
                optimized, naive,
                "Optimized and naive should match for n={}, k={}, scenario={}",
                n, k, scenario
            );
        }
    }
