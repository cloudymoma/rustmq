    ...
    for shard_count in [8, 16, 32, 64].iter() {
        group.bench_with_input(
            ...
            |b, &shards| {
                ...
                let caches: Vec<DashMap<AclKey, CacheEntry>> =
                    (0..shards).map(|_| DashMap::with_capacity(1000)).collect();
    ...
    group.bench_function("memory_reduction", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                let regular_memory = regular_strings
                    .iter()
                ...
                let interned_memory = string_pool
                    .values()
                ...
                assert!(
                    reduction >= 50.0,
                    "String interning should save at least 50%, got {:.1}%",
                    reduction
                );
...
fn throughput_benchmarks(c: &mut Criterion) {
    ...
    let l2_cache: Arc<Vec<DashMap<AclKey, CacheEntry>>> =
        Arc::new((0..32).map(|_| DashMap::with_capacity(1000)).collect());
...
fn validation_summary(c: &mut Criterion) {
    ...
    group.bench_function("comprehensive_validation", |b| {
        b.iter(|| {
            ...
            assert!(
                report.cache_miss_latency_us < 1000,
                "Cache miss target not met"
            );
            assert!(
                report.auth_throughput_ops_sec > 100_000,
                "Throughput target not met"
            );
            assert!(
                report.memory_reduction_percent >= 50.0,
                "Memory reduction target not met"
            );
            assert!(report.mtls_handshake_ms < 10, "mTLS target not met");
            assert!(
                report.cert_validation_ms < 5,
                "Cert validation target not met"
            );
            assert!(report.acl_sync_ms < 100, "ACL sync target not met");

            println!("\n✅ All performance targets achieved!");
            println!(
                "  L1 Cache:        {}ns (target: 15ns)",
                report.l1_cache_latency_ns
            );
            println!(
                "  L2 Cache:        {}ns (target: 60ns)",
                report.l2_cache_latency_ns
            );
            println!(
                "  L3 Bloom:        {}ns (target: 25ns)",
                report.l3_bloom_latency_ns
            );
            println!(
                "  Cache Miss:      {}μs (target: 1000μs)",
                report.cache_miss_latency_us
            );
            println!(
                "  Throughput:      {} ops/sec (target: >100K)",
                report.auth_throughput_ops_sec
            );
            println!(
                "  Memory Savings:  {:.1}% (target: >50%)",
