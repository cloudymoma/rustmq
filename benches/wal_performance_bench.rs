...
use rustmq::storage::{
    AlignedBufferPool, DirectIOWal, OptimizedDirectIOWal, WalFactory, traits::WriteAheadLog,
};
...
async fn setup_optimized_wal(config: WalConfig) -> OptimizedDirectIOWal {
    let buffer_pool = Arc::new(AlignedBufferPool::new(4096, 100));
    OptimizedDirectIOWal::new(config, buffer_pool)
        .await
        .unwrap()
}
        ...
        b.iter_custom(|iters| {
            rt.block_on(async {
                ...
                for _ in 0..iters {
                    ...
                    let _wal = black_box(
                        WalFactory::create_optimal_wal(config, buffer_pool)
                            .await
                            .unwrap(),
                    );
                }
