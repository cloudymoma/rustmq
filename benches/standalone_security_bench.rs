...
fn benchmark_l1_cache(c: &mut Criterion) {
    ...
    let cache_size = 1000;
    let cache = Arc::new(RwLock::new(LruCache::<AclKey, bool>::new(
    ...
    group.bench_function("mixed_hit_rate_90_percent", |b| {
        let hot_keys: Vec<AclKey> = (0..10)
            .map(|i| AclKey::new(&format!("hot_user_{}", i), "hot_topic", "READ"))
            .collect();

        let cold_keys: Vec<AclKey> = (0..100)
            .map(|i| AclKey::new(&format!("cold_user_{}", i), "cold_topic", "READ"))
            .collect();
...
fn benchmark_l2_cache(c: &mut Criterion) {
    ...
    group.bench_function("sharded_lookup", |b| {
        let test_keys: Vec<AclKey> = (0..100)
            .map(|i| {
                AclKey::new(
                    &format!("principal_{}", i),
                    &format!("resource_{}", i),
...
fn benchmark_l3_bloom_filter(c: &mut Criterion) {
    ...
    group.bench_function("negative_cache_check", |b| {
        let test_keys: Vec<String> = (0..1000)
            .map(|i| format!("denied_principal_{}_resource_{}", i * 100, i * 100))
            .collect();
    ...
    group.bench_function("memory_reduction", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                let regular_memory: usize = regular_storage
                    .iter()
                ...
                for i in 0..TOTAL_ENTRIES {
                    ...
                    let principal = intern_pool
                        .entry(principal_str.clone())
                        .or_insert_with(|| Arc::from(principal_str.as_str()))
                        .clone();

                    let resource = intern_pool
                        .entry(resource_str.clone())
                ...
                let interned_memory: usize = intern_pool
                    .values()
                ...
                let reduction =
                    ((regular_memory - interned_memory) as f64 / regular_memory as f64) * 100.0;

                // Validate we achieve target reduction
                assert!(
                    reduction >= 45.0,
                    "String interning should save at least 45%, got {:.1}%",
                    reduction
                );
...
fn benchmark_validation_summary(c: &mut Criterion) {
    ...
    group.bench_function("validate_all_targets", |b| {
        b.iter(|| {
            ...
            assert!(
                results.l1_cache_latency_ns <= 15,
                "L1 cache exceeds target: {}ns > 15ns",
                results.l1_cache_latency_ns
            );

            assert!(
                results.l2_cache_latency_ns <= 60,
                "L2 cache exceeds target: {}ns > 60ns",
                results.l2_cache_latency_ns
            );

            assert!(
                results.l3_bloom_latency_ns <= 25,
                "L3 bloom exceeds target: {}ns > 25ns",
                results.l3_bloom_latency_ns
            );

            assert!(
                results.cache_miss_latency_us < 1000,
                "Cache miss exceeds target: {}μs > 1000μs",
                results.cache_miss_latency_us
            );

            assert!(
                results.throughput_ops_sec > 100_000,
                "Throughput below target: {} < 100K ops/sec",
                results.throughput_ops_sec
            );

            assert!(
                results.memory_reduction_percent >= 45.0,
                "Memory reduction below target: {:.1}% < 45%",
