...
use rustmq::security::tls::{
    CaSettings, CertificateManager, EnhancedCertificateManagementConfig, TlsClientConfig,
    TlsConfig, TlsMode, TlsServerConfig,
};
use std::sync::Arc;
...
async fn create_test_auth_manager() -> (AuthenticationManager, TempDir, Vec<Vec<u8>>) {
    ...
    let basic_config = CertificateManagementConfig {
        ca_cert_path: temp_dir
            .path()
            .join("ca-cert.pem")
            .to_string_lossy()
            .to_string(),
        ca_key_path: temp_dir
            .path()
            .join("ca-key.pem")
            .to_string_lossy()
            .to_string(),
...
fn bench_certificate_validation_performance(c: &mut Criterion) {
    ...
    // Setup test data
    let (auth_manager, _temp_dir, test_certificates) =
        rt.block_on(async { create_test_auth_manager().await });
    ...
    group.bench_function("warm_validation", |b| {
        b.iter(|| {
            rt.block_on(async {
                auth_manager
                    .validate_certificate(criterion::black_box(warm_cert))
                    .await
            })
        })
    });
    ...
    group.bench_function("fingerprint_calculation", |b| {
        b.iter(|| {
            let cert = CertificateDer::from(criterion::black_box(warm_cert.clone()));
    ...
    c.bench_function("timing_resistance_overhead", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                auth_manager
                    .validate_certificate(criterion::black_box(test_cert_der))
                    .await
            })
        })
    });
